...
Create a run note under `docs/` named `runtime-ui-tail-cleanup-run-YYYY-MM-DD.md`. It must include `## Status` and use `FAIL` until fixes and focused live verification pass.
...
- `docs/workflows/cavalry-full-ui-100/prompts/07-runtime-capture-toolchain.md`
...
- The run note in `docs/` contains command outputs, classification table, screenshot evidence summary, and final `PASS`, `FAIL`, or `BLOCKED` status.
