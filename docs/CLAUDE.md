...
runtime-ui-live-capture-workflow.md: Runtime UI live 抓取流程，区分调试/安装包启动路径、Cavalry 窗口截图证据、`widgetAt(cursor)` 坐标反查、Qt item model dump、注入后中文 session、英文 dump-only 基线与 ExtensionLayer 自绘提示盲区，并规定全量复抓、增量修复、coverage 对比路径。
