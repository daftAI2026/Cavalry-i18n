...
audits/macos-app-management-lifecycle-lessons-2026-08-31.md: macOS App Management 生命周期与签名副作用复盘；以具体 TCC service 和系统 Quit & Reopen 证据冻结 fresh-session，以隔离 A/B 纠正首次主动 handoff 为真实 typed denial 后才恢复，并要求三个脚本入口外置签名组件纳入 journal/官方恢复且只能在精确旧残留证据下清理。
...
UI 知识归属:
可迁移的组件源码适配、所有权和证据规则归 `component-source-adaptation-protocol.md`；当前 UI 几何、平台外壳和验收边界归 `audits/switcher-ui-final-build-2026-08-28.md`；本轮执行状态与下一动作归 `roadmap/switcher-update-release-event-ledger.md`。三者互相链接但不复制，事件簿不承载长期方法论，视觉合同也不冒充真机证据。
...
2026-08-31: 新增 macOS App Management 原生实施审计，裁决以 Rust session/事务 + Objective-C++ AppKit shim 为最短路径，不引入 Swift、不增加第十条 command，并冻结 CSS→AppKit 坐标、per-screen non-key NSPanel、live NSDraggingSession、真实 apply reverse/cleanup、Reduce Motion 与最终 Info.plist readback 边界。
2026-08-31: 新增 macOS App Management 生命周期复盘；本机系统资源证明 `SystemPolicyAppBundles` 首次启用要求退出后生效并提供 Quit & Reopen，纠正从 Accessibility/Screen Recording 同进程验证类推 App Management 的错误；静态证据不冒充系统已成功重开，产品冻结 Later 明确提示重开、系统若重开则 fresh session 且不持久化旧任务的最短决策。
