...
2026-08-29: 新增 Switcher 反馈语义与四语审阅目录，并把 idle 单轨居中、首尾 Message/中段 Marker 三轨、非阻塞 text delta、8px fade 与 live-edge 接入真实 renderer；仅无 warning 的 Apply/Restore 完整路径输出本地化结果，Updater 跨重启结果、Toast 与文件级 detail 继续按真实证据保持 Blocked/未实现。
