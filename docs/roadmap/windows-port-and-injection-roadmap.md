...
5. **显示层边界**：主动翻译既有和动态菜单、动作、窗口标题与受控显示属性；不修改输入值、item model、Time Editor 或其他模型身份数据。ExtensionLayer 自绘文本只允许针对已确认的调用点使用精确 IAT 白名单，其他自绘路径保持英文，禁止宽泛 hook。
...
| W2 generic plugin | 已实现 | Qt 6.6.3 x64 MSVC 编译、CTest、Tauri resource 与 NSIS 构建；无第二套 Qt runtime | 真实 Cavalry 的首帧、动态菜单与显示白名单截图 |
