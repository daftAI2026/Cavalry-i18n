...
5. **显示层边界**：主动翻译既有和动态菜单、动作、窗口标题与受控显示属性；不修改输入值、item model、Time Editor 或其他模型身份数据。ExtensionLayer 只保留四条实证边界：helper、placeholder、MessageBar 与 text-path；其中 MessageBar 仅批准 history/live 两个 `QTextEdit::append` return 和单条 Pencil HTML 尾部正文，明确排除 `js_logger`；text-path 的十五条静态 source 只走 canonical caller，动态 `Pitch Radius: <int>` 只走 PrimitiveTool 首行/后续行两个 caller，并保留 canonical 32-bit 数值后缀。其他自绘或日志路径保持英文，禁止宽泛 hook。
...
| W2 generic plugin | 已实现 | Qt 6.6.3 x64 MSVC 编译、8 项 CTest、Tauri resource 与 NSIS 构建；四条 ExtensionLayer 边界均有 ABI 合同，动态 Pitch 生产/消费链有真实 vendor 合同，且无第二套 Qt runtime | 真实 Cavalry 的首帧、动态菜单、Pencil 警告、Pitch bit 15 与显示白名单截图 |
