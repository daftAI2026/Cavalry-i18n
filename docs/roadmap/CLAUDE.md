...
macos-app-management-handoff-animation.md: macOS App Management 单权限授权引导的实施与证据账本；冻结 typed 拒绝→设置定位→每屏快照 handoff→真实 file-URL 拖拽→写事务重试/reverse，记录九命令内 renderer/Rust/AppKit 生产实现，并把 packaged 首次授权、多屏/混合倍率与 Reduce Motion 继续留作不可伪造的 live 门。
