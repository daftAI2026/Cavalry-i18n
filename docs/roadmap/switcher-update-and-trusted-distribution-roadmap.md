...
状态: Active
实现状态: R0 行为与视觉几何已完成；R1 Rust/bridge/renderer、最终公钥/endpoint、GitHub updater Secrets、manifest producer 与 ad-hoc tag 发布闭包已实现，受保护 macOS 双架构 updater 签名 smoke 已通过；当前 HEAD 重打包、Windows updater/真机与跨版本实机证据仍待完成
记录日期: 2026-08-28
最近事实核对: 2026-09-01
目标 Cavalry: 2.7.2
...
记录 **Cavalry Language Switcher 自身**的更新提示与安全更新能力。当前仓库已实现 R0 界面，并落地 R1 的官方 Tauri updater plugin、脱敏 Rust commands、冻结 bridge 与 renderer 确认/安装状态机，以及 `latest.json`/signature/九资产的 tag 发布闭包。最终 updater 公钥与 GitHub `latest.json` HTTPS endpoint 已写入共享 Tauri 配置；截至 2026-09-01，GitHub 最新公开 Release 仍只有三项人工安装资产，没有 `latest.json` 或 updater artifacts，因此已发布版本的检查会失败关闭并保持图标隐藏，直到首个 updater-enabled Release 真实发布。

在 R1 分发闭环完成前，用户仍按现有流程手动下载对应的 macOS DMG 或 Windows NSIS 安装器。当前公开 Release 仍只有三项人工安装资产；下一个 updater-enabled tag 的代码契约已扩展为三项人工安装资产加六项 updater 资产。成熟的 ad-hoc tag 步骤已经同步进入 `LOCAL_BUILD_SOP.md`，但尚未发生的真实 tag 与跨版本结果仍只记为待验收，不能提前写成 PASS。
...
- preview 分支不触发 bridge/network；生产分支只在 Rust 返回可用 Update 时展示入口。artifact 的 Ed25519 签名在后续下载/安装事务中验证，检查阶段不能虚构“已验证”。
...
- 合同：`tools/check_renderer_contract.js` 与 `tools/check_tauri_bridge_runtime.js` 覆盖 exact SVG path、20/24px 几何、四语 tooltip、默认隐藏、preview 状态，以及 updater 请求只经冻结 bridge/Rust command 的边界。
...
因此，Apple Developer ID/notarization 与 Tauri updater 签名是不同事项；现有 acceptance attestation、release seal、SHA-256 也不能替代 updater `.sig`。当前 tag workflow 可以分发 ad-hoc macOS 包，但这种包没有 Apple 平台身份，不能据此宣称 Developer ID 公证或 Apple 认可的可信分发。
...
- 本文记录已落地的 R0 行为边界、R1 代码、最终公钥/endpoint、受保护 updater Secrets 与发布门基础；这不代表真实 tag 资产、跨版本实机证据或平台分发身份已经完成。
- 本文不把 macOS Developer ID/notarization、Windows Authenticode 或 Tauri updater artifacts 写成当前已发布 Release 的事实；当前公开 Release 仍没有 updater 资产。
- 本文同步当前 ad-hoc release SOP，但不授权跳过现有 acceptance、provenance、attestation 或 seal 门禁。
- R1 发布闭包仍保持 fail-closed：没有绑定当前候选的真实 acceptance、九资产与 exact readback 时，tag 不得创建；R0 不自动升级，也不替代这些前置工作。
