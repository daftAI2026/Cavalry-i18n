...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | 生产 renderer 已把有界事件框实现为 idle 单轨双轴居中，以及 running 的 fade 外任务引言 Message / 中段 Marker scroll-fade / fade 外 Apply-Restore 整体结果 Message；仅首尾 Message 按 `word + trailing whitespace` text delta 更新同一节点且不阻塞事务。Marker 严格对齐 shadcn `4.19.0` 的 16px/8px/14px 结构与原始 shimmer 算法，并以表现队列让已到达的快事件至少保留 360ms running、相邻新行间隔 120ms；慢后端不额外等待，error 立即同步前序事实并抢占，reduced-motion 归零。中段 fade 深度为 `8px`，用户滚离 live edge 后新增事件不抢位。Apply/Restore 只有四阶段完整完成且无 warning 才输出带四语标点的结果；Updater 没有跨重启完成凭据，仍不伪造结语。查看版新增错误立即打断场景；Toast 与文件级 detail 仍未实现，后者受限于 Apply Channel 只有 phase/state。 | 拉起当前 macOS native dev 审核三轨几何与动画；随后为文件级说明设计受控 Rust detail code，并冻结 Toast 停留、队列、键盘与无障碍规则；预览不作为 packaged/release 证据 |
...
8. 任务反馈视窗负责当前动作的过程、持久阻塞与结果输出；idle 任务邀请双轴居中，running 固定首尾 Message 并让中段 Marker 从顶部累积，仅在读者处于 live edge 时跟随，内部滚动与 8px scroll-fade 不遮蔽首尾。toast 不重复常驻事实；必要确认、权限和危险操作才使用 AlertDialog。底层异常详情不直接进入用户文案。
