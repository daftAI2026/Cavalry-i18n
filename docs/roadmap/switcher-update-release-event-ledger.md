...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | 生产 renderer 已把有界事件框实现为 idle 单轨双轴居中，以及 running 的 fade 外任务引言 Message / 中段 Marker scroll-fade / fade 外 Switch-Restore 整体结果 Message；仅首尾 Message 按 `word + trailing whitespace` text delta 更新同一节点且不阻塞事务。Marker 严格对齐 shadcn `4.19.0` 的 16px/8px/14px 结构与原始 shimmer 算法，并以表现队列让已到达的快事件至少保留 360ms running、相邻新行间隔 120ms；慢后端不额外等待，error 立即同步前序事实并抢占，reduced-motion 归零。warning/error 继续使用同一 Marker 排布，只分别给 warningCircle/errorCircle 使用 Vercel amber 与项目 danger token，并把标题提升到中性前景；说明、事件行和任务外框保持次级/中性，既增加阻断辨识度，又不把持久事件伪装成第二个 Alert。UI Review 不再拥有反馈原型副本，而以同一生产 renderer 接收 fixture Channel，因此两端天然共用 176px 任务框、12px padding、10px 圆角、94px 中段视窗与 8px fade；工作台已集中提供未找到、官方/翻译、Windows clean、重装、启动恢复、权限、成功、warning、失败、Updater 与 About 场景，并从生产真相动态生成反馈四语、语义图标和 Badge 三类总览。四个单行阶段完成后只滚动 10px。Updater 的下载阶段是唯一默认可见的阶段次行：百分比从真实字节比值更新，进入 installing 时稳定保留 `100%`，再另起验证安装阶段；文件名、临时路径与未启用的文件级细节不投影。更新可用由绿色标题栏入口持久承载，Tooltip 解释、`aria-live` 公告，点击后由 AlertDialog 决策，不重复 Toast。macOS 启动状态不写 Cavalry 权限探针，`appManagementGranted=null` 仍只表示未知；2026-09-02 的新 bundle id、独立 state 与 disposable Cavalry A/B 证明当前环境可无系统提示提交真实事务，因此 state-dir 不再投影前置权限 AlertDialog，Switch/Restore 始终先执行安全事务，只有回滚后保留的 typed `PermissionDenied` 才进入 handoff。权限拒绝不再清空已完成阶段，而把真正失败的 phase 原位收敛为链尾阻塞项“需要系统权限 / 允许语言切换器修改 Cavalry，然后重试”，再由 AlertDialog 分别说明 macOS System Settings 或 Windows UAC 的恢复动作；macOS file-URL drop 只运行一次同进程真实 oracle；已经成功展示过的安装验证/恢复文件前置阶段不重复成行，真实失败仍投影；若仍拒绝则 cleanup helper、隐藏 Retry 并显示“重新打开语言切换器”，不合成 resume 或自动续跑旧任务；说明行属于事件真实高度，DOM 更新后必须在下一布局帧复测 `scrollHeight`，只有仍处于 live edge 时才把旧阶段向上推。阶段失败不再拼接“桌面服务不可用”，该文案只保留给真实 bridge/IPC rejection。localhost 生产场景实测 `clientHeight=78 / scrollHeight=104 / maxScroll=26 / scrollTop=26`，renderer/bridge/完整 contracts 通过。未选择安装、必须重装、Cavalry 仍运行等持久事实只留在 Activity；生产 Toast 仅接 About/固定项目链接失败，默认 5000ms/3 条并对 hover、焦点和窗口失焦暂停。用户滚离 live edge 后新增事件不抢位；首尾固定 Message 的显隐、换行或流式增量都会重新测量中段高度。Switch/Restore 只有四阶段完整完成且无 warning 才输出带四语标点的结果；Updater 没有跨重启完成凭据，仍不伪造结语。 | 拉起当前 macOS native dev 审核 warning/error Marker、下载两行稳定记录与三轨动画；其余任务型 Toast 依据语义裁决保持不启用；文件级 detail 继续保持未启用，UI Review 不作为 packaged/release 证据 |
| E24 | macOS App Management 授权 handoff 动画 | In progress | 当前代码与 README 共同冻结最短事实：Gatekeeper/ad-hoc 是分发信任；App Management 是写入被系统拒绝时唯一相关的 TCC 恢复路径，但不得预判为每台机器都必需。2026-09-02 的隔离 A/B 使用新 Switcher bundle id、独立 state 与保持 Developer ID seal 的 disposable Cavalry clone，真实事务返回 `ok=true / permissionRequired=false` 且无系统提示，证伪首次主动 handoff。macOS status 不探测 TCC Granted，也不再投影“handoff 尚未展示”；Switch/Restore 直接执行可回滚事务，只有 typed denial 才打开设置，事务 commit 仍是唯一成功 oracle。匿名锁定样本已逐调用链证明：每个 request 只有一次 forward；飞行代理与落稳后的 hosted drag source 是两套对象；coordinator 用 checked continuation 等待真实 `NSDraggingSession` 的 copy drop；循环的是独立箭头而非 app/假光标。目标样本二次反汇编进一步确认 drag input/visual：`mouseDown:` 直接建立系统 session，`willBeginAtPoint:` 只隐藏 live row；独立 `appRowView.bounds` 经 `bitmapImageRepForCachingDisplayInRect`、`cacheDisplayInRect` 与 `NSImage.addRepresentation` 生成整行快照，再交给 `NSDraggingItem.setDraggingFrame:contents:`，兄弟 `NSBox` 容器的边框与底色不进入 drag image；因此生产与工作台均改为透明底的整条 App row snapshot，生产不再复制公开样本的自定义 4pt 门槛与 56pt Logo drag image。当前参数闭合为 response/damping `0.72/1.0`、50pt apex、线性尺寸/圆角、`1-p/p` opacity、12pt 对向 blur、三层 shadow/0.5pt stroke，以及箭头首延迟 0.5s、stretch 0.25s、idle 4s、`1/200/11` spring。具体样本身份、地址、摘要和 raster 只保存在兄弟 reference。公开源码复核进一步锁定 CGWindow 30Hz/12 次丢失阈值、按屏幕最大交叠完成 CG→AppKit point 映射、56pt/4pt 真实拖拽阈值，并证明公开样本只靠 AppKit backing 处理倍率、没有私有样本的每屏 replicant。工作台本机参考只走两个固定只读路由，可由 `CAVALRY_UI_REVIEW_REFERENCE_ROOT` 显式指向外部 ignored 证据目录；真实 System Settings 图只保留 Switcher 行，账户头像、侧栏和其他 App 在截图源头排除，私有 Raster 与系统截图均不进入当前仓库或 bundle。`/handoff` 已拆成导航壳、结构/样式、行为三职责并全部低于 800 行；它从真实 `#modalPrimaryButton` capture，单次 morph 到独立 helper snapshot，随后显示本项目 Phosphor ArrowUp 与可拖 app row；HTML copy drop/取消/已有行/返回重试均不制造 Granted，Reduce Motion 直接静态交接。Playwright 在 UI Review revision `mtgsrup7.mj` 上重新逐分支实跑；审查先发现并修复 reset 后旧 source document 短暂可点的导航竞态，现在只有新 document 的真实权限动作重新出现才开放交接。Reduce Motion 直接 helper/即时 cleanup；该 revision 的仍拒绝分支曾保留 helper，现已由重开提示加 cleanup 取代；其他 typed error reverse 后清层；成功 reverse 记录 66 个 RAF 样本/1084ms，目标 opacity `1→0.001` 单调收敛，中点 `p=0.5089`，双图互补和 12px 对向 blur 误差分别小于 `5.1e-7`/`5.1e-5`。真实 `dragTo` 闭合 `appDragStarted→appDropAccepted→retryRequested→operationVerified→handoffDismissed`，目标行 checked、helper 清理，console 0 error/0 warning；renderer contract 9/9 PASS。审查页旧资源根因不是 Chrome cache，而是 server 启动时解构并长期持有 Node `require` cache，同时 `/revision` 只观察 renderer；入口现已在每次工作台/handoff/catalog 请求前失效四个审查模块，600ms revision 同时覆盖 renderer 与四模块，HTTP 继续 `no-store`，后续 iframe 会自动刷新而不依赖人工重启。R2/R3/R4 生产源码现已沿该边界落地：保持九命令，点击时在 `closeModal()` 前冻结 trigger rect 与 CSS viewport，既有 `open_privacy_security` 启动 per-session Channel；独立 Rust/AppKit owner 完成 source capture、CGWindow 跟踪、每屏完整 `screenFrame` non-key/non-main replicant、独立非激活 Arrow child-panel 上的项目箭头、真实 file-URL `NSDraggingSession` 与幂等 cleanup，renderer 只在 `retryRequested` 后运行一次同进程 oracle，并把同一 session 在前次事务完成前重复到达的 Retry/drop 折叠为一次；真实 apply 成功才 reverse，任何失败均 cleanup，再次 PermissionDenied 则在 Activity 链尾提示重新打开语言切换器。四语 `NSAppBundlesUsageDescription`、最终 ad-hoc App/DMG、`CodeResources`、strict codesign 与 DMG 安装态 seal 均已回读通过，`window_chrome`、固定 URL opener 和权限 oracle 未扩责。仓库外 native harness 直编同一 `.m` 后在真实 System Settings 前发现并修复箭头/说明重叠、日文按钮截断与旧非物理箭头；四语 2x helper、`320×200` panel、`1412×485` replicant 和 `1/200/11` overshoot/回摆均已由 WindowServer/PNG 只读采样证明，未写 TCC；helper 呈现后 frontmost 仍为 `com.apple.systempreferences`，本进程 `keyWindow=none / sourceKey=0`，证明 nonactivating panel 未抢前台。同一 harness 随后只调用生产 `finish(true)`：WindowServer 从 helper 切入 7 帧 reverse replicant，第 9 帧起只剩 source，native 回读 `outcome=0 / terminal=1`，证明 reverse completion 与 overlay cleanup 同源闭合而非工作台特效。单屏失败分支继续证明无 source 时静态显示 helper、设置关闭时回送 dismissed 并清层、设置未出现时 50 次有界探测后回送 error 且无孤儿 panel；对应源码合同 4/4 PASS。二次目标复审又把旧走廊切片窗口纠正为每屏完整 `screenFrame` panel；旧 `1412×485` WindowServer 记录只保留为 shared-element 历史证据，不再冒充当前拓扑 live PASS。进程内 Reduce Motion 替身再以 600 次/10ms WindowServer 采样证明有 source 时直接静态 helper、无 forward replicant，`finish(true)` 后第一个后续采样即清层且无 reverse replicant；该结果只闭合生产分支，不冒充真实系统偏好 PASS。此前为该动画扩张出的独立账户、官方 DMG、schema recorder、seal 与首次 TCC release 证明已删除：它们不改变产品行为，只制造第二套发布门。当前实现只保留两条必要的 fail-closed 约束：macOS 深层 `PermissionDenied` 在回滚后保持 typed；copy drop 的释放点必须位于实时 System Settings 整窗内才请求重试。 本轮继续闭合权限生命周期：本机 macOS 27 `SecurityPrivacyExtension` 对 `SystemPolicyAppBundles` 明确给出 `APPLICATION_BUNDLES_QUIT_ALLOW_TITLE`、`QUIT_APP = Quit & Reopen` 与 `QUIT_LATER = Later`，证明首次启用权限后当前 Switcher 必须退出，新权限才生效；系统提供退出并重新打开动作，但静态证据不证明每次都成功重开。产品不复制该 restart，也不把旧任务持久化：若系统成功重开，新会话按普通首屏开始，由用户重新发起 Switch/Restore；drop 后只分配一个 `attemptId` 运行同进程真实 oracle，若仍拒绝则清理 helper 并显示“重新打开语言切换器”，Later 不再继续 Retry，也不插入“继续刚才的任务”或“权限已验证”合成事件。Rust 仍以受保护写事务 commit 作为唯一 App Management oracle；同进程成功才 reverse，再进入与授权无关的 Cavalry 打开阶段。公开 `p5` 与当前权限代码均无主动 Switcher restart；仓库唯一 `app.restart()` 只属于 updater 安装完成后的版本切换。 | 本轮 Node/Rust targeted contracts、完整 Rust tests 与浏览器成功/仍拒绝/其他错误逐分支均已通过；下一步在用户安排实机检查时重建包含本轮整行 drag/full-screen replicant 修正的 macOS 包，并验证当前账户可到达的真实 helper/drop/oracle/reverse、Quit & Reopen 新 PID/新会话与 Later 重开提示；首次拒绝与多屏/Space 仍只按实际可达环境验收，不另造 release 门 |
