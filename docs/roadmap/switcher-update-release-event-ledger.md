...
| E10 | macOS 实机打包证据 | In progress | 旧 `333×420`、旧 package 与底部贴齐截图均已作废。当前源码于 2026-08-29 17:40 强制重新嵌入 renderer 并重启 native dev，截图 `/tmp/cavalry-current-final.png` 显示 `400×480` 主窗、20px 内容边距、事件框 12px 内边距，以及真实安装状态触发的单色 `Reinstall Cavalry` 持久 Marker；这只证明当前 native blocker 投影。查看版新增正常四阶段与错误立即打断动画，但仍只是交互草模，不能证明真实运行态 shimmer/节奏、溢出滚动、current package 或 manual smoke。 | 下一步以 disposable Cavalry 输入触发真实任务 Channel，复核逐行节奏、错误抢占、触底推进与 scroll-fade，再重新生成当前源码 package 并运行 ignored manual smoke；native blocker、preview 与 package 证据继续分开陈述 |
...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | 生产 renderer 已把有界事件框实现为 idle 单轨双轴居中，以及 running 的 fade 外任务引言 Message / 中段 Marker scroll-fade / fade 外 Switch-Restore 整体结果 Message；仅首尾 Message 按 `word + trailing whitespace` text delta 更新同一节点且不阻塞事务。Marker 严格对齐 shadcn `4.19.0` 的 16px/8px/14px 结构与原始 shimmer 算法，并以表现队列让已到达的快事件至少保留 360ms running、相邻新行间隔 120ms；慢后端不额外等待，error 立即同步前序事实并抢占，reduced-motion 归零。生产页现与反馈原型共用同一边缘语义：中段 fade 深度为 `8px`，`data-at-start` / `data-at-end` 直接决定哪一侧收口，不再照搬 shadcn 默认 `96px` scroll-timeline 行程；安装摘要以显式 16/20 行盒收敛为 68px，大容器统一 10px 圆角，最终任务框为 172px、中段视窗为 90px，四阶段完成后只滚动 14px，不再因上游多占 7px 而完整吞掉第一行。用户滚离 live edge 后新增事件不抢位；首尾固定 Message 的显隐、换行和流式增量都会重新测量中段高度，避免结果句出现后已经溢出却仍保留 `data-overflowing=false`。Switch/Restore 只有四阶段完整完成且无 warning 才输出带四语标点的结果；内部 `restartCavalry` phase 对用户显示为“打开 Cavalry”。普通完成态已移除隐藏权限轨道残留的 8px 伪间距，结语落在真实 12px panel padding；Updater 没有跨重启完成凭据，仍不伪造结语。Toast 与文件级 detail 仍未实现，后者受限于 Apply Channel 只有 phase/state。 | 拉起当前 macOS native dev 审核直接 Switch、三轨几何与动画；随后为文件级说明设计受控 Rust detail code，并冻结 Toast 停留、队列、键盘与无障碍规则；预览不作为 packaged/release 证据 |
