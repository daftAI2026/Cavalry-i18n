...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | `docs/audits/switcher-feedback-copy-catalog-2026-08-29.md` 已按当前 handler、稳定 code、Apply 四阶段和 Updater 三阶段建立完整归属与四语审阅面；Event 负责持续任务和持久恢复，AlertDialog 负责明确选择，Base Toast 负责瞬时摘要与真实 Action。交互预览已加入居中 idle、确认后的 Message-like 语义 chunk 引言、Marker 次行原位轮换、Apply/Restore 整体成功结语及 live-edge 跟随；Toast 保持右下上浮。审计确认 Apply Channel 当前只有 phase/state，所以预览文件名只是层级草模，不是生产事实；Updater 当前进程会在安装后退出，没有跨重启完成凭据时不伪造成功结语。 | 用户审阅文案目录与动画；随后先接入 idle/引言/Apply-Restore 结语/live-edge，再为文件级说明设计受控 Rust detail code，最后冻结 Toast 停留、队列、键盘与无障碍规则；临时动画不作为 packaged/release 证据 |
