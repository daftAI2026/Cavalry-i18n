...
| E07 | shadcn / Base UI 源码与状态机审计 | Done | `select-control.js` 保留 Base UI Select 的 placeholder/open/active/selected 与键盘/ARIA 状态；初始占位不等于 selection，用户明确 commit 前 Switch 禁用。弹层只在开启瞬间定位，空值时由只读 placeholder 行锚定 Trigger、真实选值时由 selected item 锚定，pointer hover 只改 active item 而不重算位置。任务事件实现对照 shadcn 官方 Marker、shimmer、scroll-fade 提交 `683a5a9b370acdb7785a0529434e6a3b8c7e0441` 与 Phosphor Regular 提交 `2b75f3ad12b420c9504ef05df8d2564a28f8500e`：外框 12px padding 后顶部起排，未溢出保持 scrollTop=0，触底后才跟随；Marker 使用 16px 单色语义图标、8px 间距、14/20 常规中性色文字，运行态 SpinnerGap + shimmer，终态原位换成验证/归档/翻译/恢复/下载/安装/重启图标；其中 Restore 精确映射 FloppyDiskBack，强调从恢复基线写回而不是泛化历史回退。Toast 另锁定 shadcn 同提交与 `@base-ui/react 1.6.0`：5 秒、3 条、hover/focus/window blur 暂停剩余时间、F6/Escape/live region，并保留 16px 组件 inset 与 20px 主网格错层。 | 保持组件投影与业务编排分离；不复制当前不需要的多选、portal 或任意日志能力，也不以 fake DOM 代替 native/live 证据 |
...
| E09 | Tauri Updater R1 | In progress | 官方 updater plugin、最终公钥/endpoint、9 条 command registry 与 Rust-only pending Update 已接入。`install_update` 不接收 renderer 的 URL/签名/版本，只消费 Rust State，并通过 camelCase Channel 发送 `downloading`、`installing`、`restarting`；下载结束回调先于签名验证，因此第二阶段文案为“正在验证并安装”，不虚构 verified。bridge 丢弃未知阶段与 URL/签名/路径/raw，`update-progress.js` 将真实事件投影为用户任务；Channel 失败不改变更新事务。Rust updater focused 8/8、renderer/bridge 39/39 与全量 Node contracts 255/255 PASS。 | 代码边界已收口；仍需 E10/E11 真实打包、签名失败及跨版本更新验证，未完成前不宣称完整 updater PASS |
| E10 | macOS 实机打包证据 | In progress | 旧 `333×420`、旧 package 与误报 `Reinstall Cavalry` 的截图均已作废。当前源码的 native dev 外框仍为 AX/CGWindow `400×485`（配置 `400×484`）；对真实旧 Switcher 管理态 `/Applications/Cavalry.app` 的只读截图 `/tmp/cavalry-managed-legacy-native.png` 显示 2.7.2/简体中文、可用 Select、可用 `Restore English`，且不再显示重装 blocker；未点击 Switch/Restore，不把状态审核升级为写事务 smoke。全量 Rust 249 PASS / 2 explicit live-artifact tests ignored。 | 仍需以 disposable Cavalry 输入触发真实 Switch/Restore Channel，复核逐行节奏、错误抢占、触底推进与受管英文事务；随后重新生成 package 并运行 ignored manual smoke |
| E11 | Windows 实机更新证据 | Pending | 已实现 Windows `decorations:false + shadow:true` 完整窗口 override、左侧 12px 标题与随后的更新入口、右侧 minimize/maximize-or-restore/close、固定 main-window bridge、最小 capability、四语可访问名称与最大化状态同步；当前 renderer/bridge 合同 39/39 PASS。macOS AppKit 交通灯仍由 `cfg(target_os = "macos")` 隔离，DWM 继续拥有外框；尚无 Windows 真机截图、Snap、scaling 或 updater 跨版本证据 | 在真实 Windows 验证 user-wide、Program Files/UAC、100/125/150% scaling、拖动/双击/缩放/Snap、高对比度、状态保留和 updater 失败回退；不得用 macOS 或 fake DOM 推断 PASS |
...
| E20 | About 与固定项目链接 | Done | 本机 `/Applications/Maipo.app` 证明 About 由自定义窗口控制器而非标准面板；实现已删除主窗口 About Dialog，改由固定 `about` label 的 Rust owner 懒创建并复用独立本地 `about.html`。窗口使用原生装饰、固定 `320×300`、非 modal、不可 resize/maximize/minimize，页面复用现有 token，展示 64px 同源项目图标、真实 `0.7.0` 版本、带 GitHub 图标的 `Cavalry-i18n` 项目行与 MIT License。bridge、command 与 privilege 三层只接受 repository/license 枚举，任意 renderer URL 的 facade 拒绝；About/默认浏览器外链失败使用本窗口 5 秒 error Toast。当前 macOS native 已从真实应用菜单拉起并截图 `/tmp/cavalry-about-native.png`，布局、原生关闭按钮及无蓝色 focus ring 均通过；未点击外链，不把该证据扩大为 Windows native PASS。 | 保持固定枚举外链与独立窗口边界；Windows 标题栏入口和系统浏览器跳转仍随 E11 在 Windows 真机复核 |
