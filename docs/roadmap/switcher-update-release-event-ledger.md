...
| E10 | macOS 实机打包证据 | In progress | 旧 `333×420`、旧 package 与底部贴齐截图均已作废。当前源码于 2026-08-29 重新编译并重启 native dev，截图 `/tmp/cavalry-native-marker-top-restarted.png` 显示 `400×480` 主窗、20px 内容边距、事件框 12px 内边距，以及单色 `Reinstall Cavalry` Marker 从框内顶部起排；独立动画 `/tmp/cavalry-feedback-flow-marker-top.png` 展示四阶段由上向下增长。前者只证明当前 native blocker 投影，后者只是交互草模，均不证明真实溢出滚动、current package 或 manual smoke。 | 下一步用可控事件序列复核触底推进与 scroll-fade，再重新生成当前源码 package，并以 disposable Cavalry 输入运行 ignored manual smoke；三类证据继续分开陈述 |
...
| E22 | 单任务流文案与紧凑布局 | Done | 页面固定为 `Switch to` → 全宽 Select → 同行 `Apply & Restart` / `Restore` → 任务事件视窗；旧 Recovery/Refresh/双 Restore 与可见 `Action/Status/Log` 标题均删除。Apply 自动建立必要恢复基线；renderer 只把真实后端边界压缩为用户可理解的“检查安装、准备恢复文件、应用/恢复、重启”及“下载、验证并安装、重启”，不暴露 snapshot/provenance/内部函数。持久阻塞不能只使用会消失的 Toast；必须立即选择的确认/权限/危险操作才使用 AlertDialog。 | 维持 UX Writing 的结果、影响、下一步顺序；后续只按真实 backend event 扩展，不把调试日志直接暴露给用户 |
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | `docs/audits/switcher-feedback-copy-catalog-2026-08-29.md` 已按当前 handler、稳定 code、Apply 四阶段和 Updater 三阶段建立完整归属与四语审阅面；Event 负责持续任务和持久恢复，AlertDialog 负责明确选择，Base Toast 负责瞬时摘要与真实 Action。交互预览已加入居中 idle，并把有界事件框拆为 fade 外的任务引言 Message、中部无边框 Marker scroll-fade、fade 外的 Apply/Restore 整体结果 Message；仅首尾 Message 按 `word + trailing whitespace` text delta 更新同一节点，不给 delta 添加入场动画，Marker 次行与主步骤统一 `14px/20px`、`8px` 节奏。中部 fade 深度收为 `8px`，live-edge 也只控制该视窗。Toast 保持右下上浮。审计确认 Apply Channel 当前只有 phase/state，所以预览文件名只是层级草模，不是生产事实；Updater 当前进程会在安装后退出，没有跨重启完成凭据时不伪造成功结语。 | 用户审阅文案目录与动画；随后先接入 idle/引言/Apply-Restore 结语/live-edge，再为文件级说明设计受控 Rust detail code，最后冻结 Toast 停留、队列、键盘与无障碍规则；临时动画不作为 packaged/release 证据 |
...
8. 任务事件视窗负责当前动作的过程、持久阻塞与结果输出；idle 只有 separator，运行时从内部 padding 后的顶部逐条向下累积，触底后旧事件才向上推进并保留内部滚动/scroll-fade。toast 不重复常驻事实；必要确认、权限和危险操作才使用 AlertDialog。底层异常详情不直接进入用户文案。
