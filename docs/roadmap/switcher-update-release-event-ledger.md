...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | 已按当前 renderer handler、稳定 error/warning code、Apply 四阶段和 Updater 三阶段建立语义矩阵；Event 负责持续任务和持久恢复，AlertDialog 负责明确选择，Toast 只负责启动指导、更新可用或独立局部操作的瞬时摘要。Toast 已冻结为窗口右下定位、由下向上进入、多条向上堆叠；组件采用 Base Toast 的可选状态图标、标题、说明、可选 Action 与关闭控件，Action 仅用于真实且安全的即时下一步。临时交互动画 `/Users/luo/Downloads/cavalry-feedback-flow-preview.html` 已加入真实产品场景变体，但不作为 packaged 或 release 证据。 | 继续冻结 Toast 的停留时间、队列/替换、键盘与无障碍规则后再接入生产；当前不提前实现生产 Toast |
