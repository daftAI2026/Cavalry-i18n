...
| E23 | Event / AlertDialog / Toast 反馈分层 | In progress | 生产 renderer 已把有界事件框实现为 idle 单轨双轴居中，以及 running 的 fade 外任务引言 Message / 中段 Marker scroll-fade / fade 外 Switch-Restore 整体结果 Message；仅首尾 Message 按 `word + trailing whitespace` text delta 更新同一节点且不阻塞事务。Marker 严格对齐 shadcn `4.19.0` 的 16px/8px/14px 结构与原始 shimmer 算法，并以表现队列让已到达的快事件至少保留 360ms running、相邻新行间隔 120ms；慢后端不额外等待，error 立即同步前序事实并抢占，reduced-motion 归零。生产页与反馈原型现共用 176px 任务框、12px padding、10px 圆角、94px 中段视窗与 8px fade；四个单行阶段完成后只滚动 10px。Updater 的下载阶段是唯一默认可见的阶段次行：百分比从真实字节比值更新，进入 installing 时稳定保留 `100%`，再另起验证安装阶段；文件名、临时路径与原型中保留的文件级中间步骤默认不投影。更新可用由绿色标题栏入口持久承载，Tooltip 解释、`aria-live` 公告，点击后由 AlertDialog 决策，不再规划重复 Toast。用户滚离 live edge 后新增事件不抢位；首尾固定 Message 的显隐、换行和流式增量都会重新测量中段高度。Switch/Restore 只有四阶段完整完成且无 warning 才输出带四语标点的结果；Updater 没有跨重启完成凭据，仍不伪造结语。 | 拉起当前 macOS native dev 审核新高度、下载两行稳定记录与三轨动画；其余 Toast 与文件级 detail 继续保持未启用，预览不作为 packaged/release 证据 |
...
current updater feedback prototype                                400×484; Activity 360×176; `/tmp/cavalry-update-feedback-176.png`
