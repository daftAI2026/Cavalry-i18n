...
macos-native-app-management-implementation-2026-08-31.md: macOS App Management handoff 原生实施边界审计，冻结 Rust/Objective-C++ 最短路径、九命令、CSS→AppKit 坐标、per-screen NSPanel、NSDraggingSession、真实 apply reverse/cleanup、Reduce Motion 与 Info.plist；后续附录记录 motion surface 单一几何 owner 及不移动静止 glyph 的箭头 overscan 修正，未知私有行为不写成事实。
macos-app-management-lifecycle-lessons-2026-08-31.md: macOS App Management 生命周期与签名副作用复盘；以具体 TCC service 和系统 Quit & Reopen 证据冻结 fresh-session，并以 2026-09-02 新 bundle id / 独立 state / disposable Cavalry A/B 纠正“首次必然先 handoff”为真实 typed denial 后才恢复；三个脚本入口外置签名组件纳入 journal/官方恢复且只能由后端按精确 preimage 静默清理，不能成为用户状态；journal 保留结构/路径/CAS 恢复但不再访问 Keychain，Switch/Restore 共用最早只读运行态门。
