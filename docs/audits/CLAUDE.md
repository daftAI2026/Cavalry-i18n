...
macos-app-management-lifecycle-lessons-2026-08-31.md: macOS App Management 生命周期与签名副作用复盘；以具体 TCC service 和系统 Quit & Reopen 证据冻结 fresh-session，并证明首次 handoff 必须早于 Cavalry mutation、三个脚本入口外置签名组件必须纳入 journal/官方恢复且只能由后端按精确 preimage 静默清理，不能成为用户状态；journal 保留结构/路径/CAS 恢复但不再访问 Keychain，Switch/Restore 共用最早只读运行态门。
