...
switcher-feedback-copy-catalog-2026-08-29.md: Switcher 四语反馈语义审阅目录，逐项区分生产 Event、必要 AlertDialog、未实现 Toast 与后端事件阻塞；冻结更新可用只由持久标题栏入口、Tooltip、`aria-live` 和点击后 AlertDialog 承担，不用 Toast 重复同一事实。
