...
| `olderVersionUnsupported` | <strong>Cavalry {version} isn’t supported</strong><br>This Switcher supports Cavalry {supportedVersion}. To use it, update Cavalry to {supportedVersion}. Your installation has not been changed. | <strong>暂不支持 Cavalry {version}</strong><br>此语言切换器支持 Cavalry {supportedVersion}。如需使用，请将 Cavalry 更新到 {supportedVersion}。当前安装未被修改。 | <strong>暫不支援 Cavalry {version}</strong><br>此語言切換器支援 Cavalry {supportedVersion}。如需使用，請將 Cavalry 更新至 {supportedVersion}。目前安裝未被修改。 | <strong>Cavalry {version} はサポートされていません</strong><br>この言語スイッチャーは Cavalry {supportedVersion} に対応しています。使用するには Cavalry を {supportedVersion} に更新してください。現在のインストールは変更されていません。 |
| `newerVersionUnsupported` | <strong>Cavalry {version} isn’t supported yet</strong><br>This Switcher supports Cavalry {supportedVersion} and won’t modify your newer installation. You can keep using Cavalry normally and install a compatible Switcher update when available. | <strong>尚未支持 Cavalry {version}</strong><br>此语言切换器目前支持 Cavalry {supportedVersion}，不会修改你的新版本安装。你可以继续正常使用 Cavalry，待兼容的语言切换器更新后再安装。 | <strong>尚未支援 Cavalry {version}</strong><br>此語言切換器目前支援 Cavalry {supportedVersion}，不會修改你的新版本安裝。你可以繼續正常使用 Cavalry，待相容的語言切換器更新後再安裝。 | <strong>Cavalry {version} はまだサポートされていません</strong><br>この言語スイッチャーは現在 Cavalry {supportedVersion} に対応しており、新しいバージョンは変更しません。Cavalry はそのまま使用し、対応する言語スイッチャーの更新をお待ちください。 |
