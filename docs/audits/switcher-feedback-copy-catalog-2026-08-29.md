...
| `restoreOutcome` | Restored English. Cavalry is now open. | 已恢复英文，Cavalry 已打开。 | 已還原英文，Cavalry 已開啟。 | 英語に戻しました。Cavalry は起動済みです。 |
| `updateOutcome` | — | — | — | — |
...
| `permissionRequired` | <strong>System permission required</strong><br>Allow the Switcher to modify Cavalry, then retry. | <strong>需要系统权限</strong><br>允许语言切换器修改 Cavalry，然后重试。 | <strong>需要系統權限</strong><br>允許語言切換器修改 Cavalry，然後重試。 | <strong>システム権限が必要です</strong><br>言語スイッチャーによる Cavalry の変更を許可してから、再試行してください。 |
| `startupRecoveryFailed` | **Couldn’t recover the interrupted operation** — Close Cavalry, then restart the Switcher. To preserve the recovery data, don’t modify the Cavalry installation folder manually. | **无法恢复中断的操作** — 请关闭 Cavalry，然后重新启动语言切换器。为避免丢失恢复信息，请勿手动修改 Cavalry 安装目录。 | **無法復原中斷的操作** — 請關閉 Cavalry，然後重新啟動語言切換器。為避免遺失復原資料，請勿手動修改 Cavalry 安裝目錄。 | **中断した操作を復旧できません** — Cavalry を終了してから、言語スイッチャーを再起動してください。復元データを保持するため、Cavalry のインストール先を手動で変更しないでください。 |
...
| `reinstallRequired` | **Reinstall Cavalry** — The Switcher can’t verify the original English files needed for recovery. Reinstall Cavalry from the official installer, then reopen the Switcher. | **重新安装 Cavalry** — 语言切换器无法验证恢复所需的原始英文文件。请使用官方安装包重新安装 Cavalry，再重新打开语言切换器。 | **重新安裝 Cavalry** — 語言切換器無法驗證復原所需的原始英文檔案。請使用官方安裝程式重新安裝 Cavalry，再重新開啟語言切換器。 | **Cavalry を再インストール** — 復元に必要な元の英語ファイルを確認できません。公式インストーラーで Cavalry を再インストールしてから、言語スイッチャーを開き直してください。 |
| `olderVersionUnsupported` | **Cavalry {version} isn’t supported** — This Switcher currently supports Cavalry {supportedVersion}. Update Cavalry to {supportedVersion}, then reopen the Switcher. Your installation has not been changed. | **暂不支持 Cavalry {version}** — 语言切换器目前支持 Cavalry {supportedVersion}。请先将 Cavalry 更新至 {supportedVersion}，再重新打开语言切换器。当前安装未被修改。 | **暫不支援 Cavalry {version}** — 語言切換器目前支援 Cavalry {supportedVersion}。請先將 Cavalry 更新至 {supportedVersion}，再重新開啟語言切換器。目前的安裝未被修改。 | **Cavalry {version} はサポートされていません** — この言語スイッチャーは現在 Cavalry {supportedVersion} に対応しています。Cavalry を {supportedVersion} に更新してから、言語スイッチャーを開き直してください。現在のインストールは変更されていません。 |
| `newerVersionUnsupported` | **Cavalry {version} isn’t supported yet** — This Switcher currently supports Cavalry {supportedVersion} and won’t modify this installation. Keep using Cavalry, then try again after a compatible Switcher update is available. | **尚未支持 Cavalry {version}** — 语言切换器目前支持 Cavalry {supportedVersion}，不会修改此安装。你可以继续使用 Cavalry；请在兼容更新发布后再试。 | **尚未支援 Cavalry {version}** — 語言切換器目前支援 Cavalry {supportedVersion}，不會修改此安裝。你可以繼續使用 Cavalry；請在相容更新推出後再試。 | **Cavalry {version} はまだサポートされていません** — この言語スイッチャーは現在 Cavalry {supportedVersion} に対応しており、このインストールは変更しません。Cavalry はそのまま使用し、対応版の更新後にもう一度お試しください。 |
| `unknownVersionUnsupported` | <strong>This Cavalry version isn’t supported</strong><br>This Switcher supports Cavalry {supportedVersion} and has not changed your installation. | <strong>暂不支持此 Cavalry 版本</strong><br>此语言切换器支持 Cavalry {supportedVersion}，当前安装未被修改。 | <strong>暫不支援此 Cavalry 版本</strong><br>此語言切換器支援 Cavalry {supportedVersion}，目前安裝未被修改。 | <strong>この Cavalry バージョンはサポートされていません</strong><br>この言語スイッチャーは Cavalry {supportedVersion} に対応しています。現在のインストールは変更されていません。 |
...
| `cavalryStillRunning` | **Cavalry is running** — Save your work and quit Cavalry, then try again. The installation has not been changed. | **Cavalry 正在运行** — 保存工作并退出 Cavalry 后重试。安装内容未被修改。 | **Cavalry 仍在執行** — 請先儲存工作並結束 Cavalry，然後重試。目前的安裝未被修改。 | **Cavalry が実行中です** — 作業を保存して Cavalry を終了してから、もう一度お試しください。インストール内容は変更されていません。 |
