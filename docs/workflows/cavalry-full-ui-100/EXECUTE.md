...
- ❌ 不允许新增、修改、依赖 `docs/libExtensionLayer-curated-ui.txt` 或同类清单
...
WORKFLOW   = REPO/docs/workflows/cavalry-full-ui-100
...
本 workflow 按 [`docs/LOCAL_BUILD_SOP.md`](../../LOCAL_BUILD_SOP.md) 执行：
...
```text
/Users/luo/Desktop/ClaudeCode/web/Cavalry-i18n/docs/workflows/cavalry-full-ui-100/
```

原因：执行 worktree 的 `docs/` 被 `.gitignore` 忽略。`SESSION_DIR/full-ui-run-record.json` 只能作为机器 run record，不能替代 `runs/YYYY-MM-DD-{gate-or-task}.md`。
每轮结束前必须同步：

```text
RUN_RECORD                           # 机器证据
MAIN_REPO/docs/workflows/.../runs/*.md # 语义证据
...
- 复制 `docs/libExtensionLayer-curated-ui.txt` 任何内容
...
ls docs/libExtensionLayer-curated-ui.txt 2>&1 || echo "curated corpus absent: OK"
