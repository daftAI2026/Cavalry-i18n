...
! rg -n 'run: npm run build$|docs/compiled-ui-source-map.json|docs/translation-whitelist.json' .github/workflows/build.yml
...
说明：`npm run build`、旧 `docs/...` artifact 路径、`99`、`0.90` 都是 fail-on-match；不能用“同时存在新旧写法”冒充通过。
