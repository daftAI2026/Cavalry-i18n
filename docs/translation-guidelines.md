...
[OUTPUT]: 对外提供简中/繁中/日语翻译、保留词、字体选择值与 JSON 默认身份保护、Add Layer 搜索/显示/身份分离契约、快捷键身份标记及零混语边界
...
JSON 节点定义中的 `type: "font"` 同样属于这个边界：其 `default` 是字体查找参数，不是显示文案，必须与英文基线完整一致（包括字体族、样式及后续新增的身份参数）。保护控件不能修复已经被翻译的默认数据；两条入口必须分别验收。
