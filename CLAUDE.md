...
src-tauri/ - Tauri 后端；分离 renderer 契约、安装真相、受控系统命令与平台运行时，macOS 首次受保护写入先进入 App Management handoff，并把脚本入口重签的三个自有外置组件纳入不暴露给用户的路径级兼容清理、回滚和 English 恢复；已证明的受管 runtime 不再以 strict codesign 纯洁性作为语言切换准入，最终签名仍是事务提交门，Windows 以无系统 caption + DWM 外框承载右侧 renderer 控件；语言事务通过四阶段 Channel 投影真实边界，Updater 只从 Rust State 消费已检查 Update；immutable snapshot、durable manifest/backup、原子 English 清理、same-EXE UAC 与 NSIS 生命周期统一控制面和数据面 (Rust, NSIS)
