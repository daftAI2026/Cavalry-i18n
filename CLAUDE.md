...
renderer/ - Tauri 前端 UI，以独立语义 token 真相源、系统字体与只消费角色的离线 HTML/CSS/JS 提供安装对象双徽章、跨平台路径中部省略、无依赖 Base UI 语义 Select/Tooltip、自动恢复基线的 Apply/平台统一 Restore 单任务流、固定项目外链 About，以及由真实 Apply/Updater Channel 驱动的 idle 居中/首尾 Message/中段 Marker 三轨任务视窗；首尾 text delta 不阻塞事务，中段复用集中 Phosphor 图标、Spinner/shimmer/8px scroll-fade 并尊重 live edge，表现队列只为已到达的快事件提供最短可读节奏且让错误立即抢占，持久阻塞留在视窗，只有确认/权限/风险进入 AlertDialog，主窗口禁止滚动，macOS 保留原生交通灯，Windows 使用右侧原生语义 caption 状态机且不接管系统外框 (HTML, CSS, JS)
