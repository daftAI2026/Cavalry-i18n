...
renderer/ - Tauri 前端 UI，以单一 token、系统字体和离线 HTML/CSS/JS 提供无描边彩色 Badge、显式占位 Select/Tooltip、直接 Switch/单一 Restore English、真实 Channel 驱动的三轨 Activity、必要 AlertDialog，以及 Base UI 对齐的外围失败 Toast；持久事实不叠 Toast，About/固定外链失败不污染任务流，窗口与平台原生 chrome 边界保持分离 (HTML, CSS, JS)
