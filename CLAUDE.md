...
renderer/ - Tauri 前端 UI，以系统字体与统一 spacing token 的离线 HTML/CSS/JS 提供安装对象双徽章、无依赖 Base UI 语义语言选择器、维护层级与四语动态语义 Alert，macOS 标题区保留系统原生交通灯 (HTML, CSS, JS)
src-tauri/ - Tauri 后端；分离 renderer 契约、安装真相、受控系统命令与平台运行时，macOS 以最小 AppKit 桥在 resize/scale 后维持原生交通灯中心，Windows 以 immutable snapshot 原字节恢复、durable manifest/backup、含 generic 的原子 English 清理、same-EXE 单次 UAC 与可选保留翻译的 NSIS 生命周期统一控制面和数据面 (Rust, NSIS)
...
src-tauri/tauri.conf.json - Tauri 共享运行配置，固定本地 CSP/updater 信任根与 460×428 macOS Overlay 原生窗口边界
