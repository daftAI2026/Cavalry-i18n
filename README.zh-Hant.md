...
- **自動尋找與還原**：尋找常見安裝位置、顯示目前語言，並準備受支援的英文還原所需資料。
- **應用程式內更新**：發現後續 Switcher 版本時提醒使用者，並在安裝前完成驗證。

## Switcher 視窗

選擇目標語言，再點選 **「切換」** 或 **「還原英文」**。目前語言旁出現可更新徽章時，可以選取它並點選 **「更新」**，重新套用完整語言修補程式，無須先切換其他語言或還原英文。只有修補程式已是最新時，目前語言才無法重複選取。操作進度和復原指引顯示在按鈕下方。

在 macOS 上，對已受管安裝再次切換語言、更新修補程式或「還原英文」，均要求有為該安裝保存且可驗證的完整 Cavalry 官方基線。若基線缺失或無法驗證，操作會在寫入前停止，並提示先從官方安裝程式重新安裝 Cavalry，再套用語言。Windows 行為不受此要求影響。

**已發佈 p8 的 Add Layer 搜尋：** 可以使用英文或目前語言搜尋。右側 Add Layer 面板中，本地語言的標題比對不一定獲得與英文名稱相同的標題優先順序。如果目標圖層排得較後，可以嘗試完整英文名稱，例如 `Text Shape`。這是已知的排序限制，不代表搜尋輸入或圖層的建立身分被改寫。

**開發分支（尚未發佈）：** 在已驗證相容的執行階段中，右側面板的目前語言標題完整比對，以及符合原廠規則的標題前綴比對，會獲得標題優先順序補充。標題其他位置的比對仍可出現在搜尋結果中，並保持原廠排序；篩選與圖層建立仍由原廠處理。無法驗證相容性時保留原廠排序，發佈前仍需完成 Windows 實機驗收。
...
在 macOS 上，**「還原英文」** 依據已保存的官方基線還原，不會替你重新安裝 Cavalry。若缺少完整、可驗證的基線，還原會在寫入前停止，並提示從官方安裝程式重新安裝 Cavalry 2.7.2 後再套用語言。
