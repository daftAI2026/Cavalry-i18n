...
[OUTPUT]: 对外提供 macOS / Windows 用户安装、当前补丁更新、搜索排序限制、开发与安全说明的繁體中文版本
...
**Add Layer 搜尋：** 可以使用英文或目前語言搜尋。右側 Add Layer 面板中，本地語言的比對結果（包含說明文字比對）不一定獲得與英文名稱相同的標題優先順序。如果目標圖層排得較後，可以嘗試完整英文名稱，例如 `Text Shape`。這是已知的排序限制，不代表搜尋輸入或圖層的建立身分被改寫。
