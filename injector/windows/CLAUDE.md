...
CMakeLists.txt: Qt 6.6.3 x64 MSVC + Windows Psapi generic plugin 构建边界；编译 display、translator、PE/IAT、ExtensionLayer 双 IAT hook、可选只读 vendor ABI/import 与 plugin smoke 合同测试，发布产物只含 `generic/cavalryi18n.dll`。
...
cavalry_i18n_display.h: 主动显示翻译接口与对象生命周期状态，明确 QWidget/QAction、已知基名数字后缀与 QComboBox DisplayRole 白名单。
cavalry_i18n_display.cpp: 幂等翻译菜单、动作、标题、已知基名数字后缀和 QComboBox 可见项，通过 `aboutToShow`/`changed`/Paint 接住首帧与动态英文写回，并隔离输入值、UserRole、currentIndex 和通用 item view。
cavalry_i18n_display_test.cpp: 三语显示层单元回归，锁定任意已知基名的空格/点数字后缀、受控 QComboBox DisplayRole 投影、自定义名称透传及 UserRole/currentIndex 不变。
...
cavalry_i18n_runtime.cpp: 读取显式语言环境并安装嵌入式 `QTranslator`，把 Show/ActionAdded/含 QComboBox 的局部 Paint 事件路由到显示翻译器，并在目标模块出现的首帧前尝试精确 hook，同时原子记录条目与 hook 状态。
...
cavalry_i18n_plugin_smoke_test.cpp: 由最小 `QApplication` 走真实 generic plugin 自动发现，验证十二个顶层菜单、数字后缀、QComboBox Show/Paint 动态恢复、显示白名单、数据隔离与当前进程 marker；不将样本当作 hook 覆盖证据。
