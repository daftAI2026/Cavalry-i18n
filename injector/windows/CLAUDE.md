...
cavalry_i18n_display.h: 主动显示翻译接口与对象生命周期状态，明确 QWidget/QAction、已知基名数字后缀、QComboBox/QTreeWidget DisplayRole、仅 `placeholderText` 受词表约束的 QLineEdit；其实际值（普通/只读名称与 parentless/重挂接查询）保持原文、厂商父系内精确 QPlainTextEdit 占位文字、exact Classic `ListWidget`/真实 viewport 的空结果 surface、真实 Assets ContextMenu 的 Replace/动态 Create 双 action 边界，以及仅供原生测试目标注入 CavalryUI `gMainWindow`/placeholder 访问的 seam。
...
cavalry_i18n_runtime.h: QPA 显式语言、可查询配置结果、主动显示刷新、聚合翻译与独立时间轴字体边界的延迟安装及 revision-driven marker 生命周期声明；产品接口不持有 Onboarding/Adjacent driver 或任何验收环境入口。
...
cavalry_i18n_plugin_smoke_test.cpp: 由最小 `QApplication` 加载真实 generic DLL，证明空 specification 即使存在遗留环境也被拒，并验证 QPA 等价显式语言、显示投影、数据隔离及翻译/字体各自的诊断 marker。
