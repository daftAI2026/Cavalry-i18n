...
CMakeLists.txt: CMake 4.2+ 与 Visual Studio 2022+ MSVC v143 的 shared Qt 6.6.3 x64 + Windows Psapi 构建边界；拒绝静态 Qt，编译 generic 翻译 runtime 与使用版本化私有 QPA 头的原厂委托代理，注册 display/hook/vendor、strict manifest 及仅显式语言入口合同，发布 `generic/cavalryi18n.dll` 和 `qpa/qwindows.dll`。
...
cavalry_i18n_runtime.h: QPA 显式语言、可查询配置结果、主动显示刷新、聚合四边界延迟安装及 revision-driven marker 生命周期声明；额外持有仅由显式同目录证据握手启用的 firstLaunch 五步验收状态与当前 Guide native HWND，第五步 ACK 后保持产品窗口且不触发完成/取消语义。
cavalry_i18n_runtime.cpp: 语言只消费 QPA 非空 specification，绝不读取 `CAVALRY_I18N_LANG`；Show/Paint 仅刷新受控显示属性（含 QPlainTextEdit 占位文字但不读正文），显式绝对 marker 下把 75ms 诊断/验收轮询器投递到 QApplication 线程后创建，避免早期插件线程尚无事件分发器；Windows Onboarding live gate 额外要求 marker 同目录的真实非链接证据目录，外部场景取证与 Windows 前台等待不占产品内部超时，收到 `foreground-ready.json` 后才启动阶段计时；随后选定进程内唯一最大可见 Cavalry 顶层 QWidget，取得 HWND 并经 `SetForegroundWindow` 后精确复核前台所有权，再投递一次原生 `Alt+H` 物化懒 Help 菜单；从 QWidget associated actions、QObject 子树、qApp 子树及 `ActionAdded` 事件中唯一解析 `objectName/data=showGuides`，或以 exact `MenuBarManager/Getting Started Guides` 译文作受控兜底，再只在元对象参数精确为 `std::string` 时语义触发 `guideSelected("firstLaunch")`，前四步调用 `nextClicked`；验收 marker 同时记录参数类型和 Onboarding QWidget inventory，每步要求唯一可见标题 QLabel 与唯一可见正文 QTextBrowser 分别精确匹配安装 catalog，随后连同顶层 native HWND 发布 ready，第五步 ACK 后直接完成而不触发任何关闭语义。
cavalry_i18n_translator.h: 嵌入式 translator 查询接口与统计边界，隔离生成表表示和运行时生命周期。
cavalry_i18n_translator.cpp: 复用共享 `generated_translations.inc`，构建精确 `(context, source)` 首条优先哈希与遵循现有显示层语义的末条覆盖 source fallback；共享策略声明的 context-only、8 条 ordinary-Qt exact-only，以及 macOS owner 已证但 Windows producer 待证的 Tag/动态 Assets 邻接 key 均不进入 fallback。
cavalry_i18n_translator_test.cpp: 三语言非空表、已证实 helper 与 ordinary-Qt 残留、编号书签、Color Settings/Mesh Explorer/单索引动态模板、LineTool 精确标签、具体 Add Layer 快捷键，以及 macOS owner 已采证但 Windows producer 待证的 Tag/动态 Assets 共享 key 之 exact-context 正例与 Unknown/null fallback 负例；同时覆盖 context-only 拒绝、普通 source fallback、未知语言和未知文本。
