...
CMakeLists.txt: Qt 6.6.3 x64 MSVC + Windows Psapi generic plugin 构建边界；编译 display、translator、PE/IAT、ExtensionLayer 四条边界、mapped Skia runtime ABI 防火墙、CJK renderer 与 process-lifetime callback snapshot，发布产物只含 `generic/cavalryi18n.dll`。
...
cavalry_i18n_extension_layer_sources.h: 不依赖 Qt 的共享文本真相，锁定九条 helper、十三条 CustomListWidget placeholder、一条 MessageBar Pencil 警告，以及十五条 text-path source：四条 viewport quality、六组 EditShapeTool 与五组 TransformTool 的英文 prefix/action。
