/**
 * [INPUT]: 依赖 CavalryEmbeddedTranslator 的精确 source/context 查询，共享选择/补全输入值保护策略，以及 Qt Widgets 的公开显示属性、QComboBox/QTreeWidget DisplayRole、QLineEdit/QPlainTextEdit、菜单事件与 Windows vendor 主窗口/Assets producer 身份
 * [OUTPUT]: 对外提供幂等的 QWidget/QAction 主动翻译、已知基名数字后缀、来源绑定的受控动态模板、真实 Assets 菜单模板投影、输入框显示值、QTreeWidget 递归 DisplayRole 刷新及测试态主窗口注入缝
 * [POS]: injector/windows 的显示层边界，保护可编辑/字体 Combo 及其后代原值和所有 CompleterLineEdit 输入，只改其外部受控可见文案、下拉框/树的 DisplayRole、词表命中的非补全 QLineEdit 与经运行时证明的厂商父系/producer 表面；未知输入、编辑器正文、UserRole、currentIndex、无关同文控件与通用 item view 保持原值
