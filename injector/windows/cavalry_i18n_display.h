 ...
 * [OUTPUT]: 对外提供幂等的 QWidget/QAction 主动翻译、已知基名数字后缀、来源绑定的受控动态模板、真实 Assets 菜单模板投影、输入框占位提示、QTreeWidget 递归 DisplayRole 刷新、exact Classic `ListWidget`/真实 viewport 选择器与 `No Results` placeholder 投影，以及测试态主窗口/placeholder 访问注入缝
 * [POS]: injector/windows 的显示层边界，保护可编辑/字体 Combo 及其后代原值和全部 QLineEdit 实际值，只改其外部受控可见文案、下拉框/树的 DisplayRole、QLineEdit 占位提示、经运行时证明的厂商父系/producer 表面和 exact Classic 空结果 placeholder；未知输入、编辑器正文、UserRole、currentIndex、无关同文控件、普通 item view 与非真实 viewport 保持原值
