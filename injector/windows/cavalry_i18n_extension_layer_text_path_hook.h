/**
 * [INPUT]: 依赖 Core::MakePathFromText 的 MSVC x64 ABI、ExtensionLayer 三处持续复核字节包络的批准 caller、二十七项翻译 source/context、已验证 CJK renderer 与嵌入 translator
...
struct CavalryTextPathHookDiagnostics final {
    ...
    std::uint32_t translatedSourceMask = 0;
    std::uint32_t fallbackSourceMask = 0;
};
