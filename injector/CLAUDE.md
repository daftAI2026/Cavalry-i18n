...
法则: 翻译表自动生成·模型 niceName 保留英文·显示层模型名可翻译·QLineEdit 显示翻译不改模型·动态菜单规则化·dylib 预编译·Qt ABI 锁定·ExtensionLayer 自绘层保留英文
