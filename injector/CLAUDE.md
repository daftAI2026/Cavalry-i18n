...
成员清单
CavalryTranslatorInjector.mm: Objective-C++ runtime 翻译注入器主源，子类化 QTranslator 拦截 Qt 菜单与普通 QWidget 翻译，并用 Mach-O `__cstring` copy-on-write 补丁命中 ExtensionLayer 自绘提示，支持定时刷新 UI 与 English dump-only 模式输出 runtime inventory。
...
法则: 翻译表自动生成·dylib 预编译·Qt ABI 锁定·自绘字面量定点补丁
