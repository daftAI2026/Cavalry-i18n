...
成员清单
CavalryTranslatorInjector.mm: Objective-C++ runtime 翻译注入器主源，子类化 QTranslator 拦截 Qt 菜单与普通 QWidget 翻译，用 display-only 模型名词典恢复属性编辑器浮动标题等 Qt 显示层翻译，监听 QLineEdit 首次绘制前与后续文本变化并以 signal-blocked 写回，通用保留自动编号、点编号与括号编号动态图层名后缀，派生运行时生成的 `X Shape` 图层名显示翻译，用 QObject property ABI-safe 读取 accessibility 字符串识别 Time Editor item view，阻止模型 niceName 与动态括号图层名经 item `setText()` 或 QAbstractItemView `DisplayRole/EditRole` 写成 CJK，QMenu `aboutToShow/ActionAdded/Show` 首次绘制前同步翻译懒加载菜单项，QDialog/ModalDialog 首次绘制前同步翻译退出确认窗与按钮，只在 `QTextEdit::append(QString)` 入口翻译 MessageBar 日志正文，保留原生弹窗 Paint/Show 动画路径，把 `Copied <object>` 与 `Undo/Redo (<operation>)` 动态消息拆成固定谓词加对象/操作名词典翻译，并集成 Undo/Redo (<operation>) 模板化匹配逻辑，复用于 MessageBar 弹窗和底部 QLabel/QStatusBar 即时状态文本，dyld 符号解析失败时提供 QTextCursor 直接插入的安全写入兜底，规则化处理动态右键菜单标签、认证倒计时状态句、冒号后缀标签与 No 前缀混合文本，支持定时刷新 UI 与带坐标父链/Qt item model/MessageBar meta-object metadata-only 的 runtime inventory，禁止在 inventory 中读取 QTextEdit 正文。ExtensionLayer 自绘层使用 Latin-only 字体，CJK 显示为 `?`，不注册 `__cstring` 补丁回调并保持英文原文。
...
法则: 翻译表自动生成·模型 niceName 保留英文·显示层模型名可翻译·Scene View 图层列表可翻译·Time Editor 自身 item view 与 model role 保留英文·QLineEdit 显示翻译不改模型·QTextEdit append 只翻译日志正文与有限日志模板·aboutToShow 菜单同步翻译·动态菜单规则化·dylib 预编译·禁止引入目标 Qt 缺失符号·Qt ABI 锁定·ExtensionLayer 自绘层保留英文且不走空补丁
