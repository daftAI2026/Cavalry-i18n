...
依赖边界:
injector 在 Cavalry 进程内通过 DYLD_INSERT_LIBRARIES 加载；它只依赖 Qt 6.6.3 runtime ABI 与 AppKit，不依赖仓库其他模块。generated_translations.inc 是 tools/*.ts 的机器投影，任何翻译变更必须通过 generate_embedded_translations.js 重生成。

法则: 翻译表自动生成·dylib 预编译·Qt ABI 锁定
