...
法则: 翻译表自动生成·模型 niceName 保留英文·显示层模型名可翻译·Scene View 图层列表可翻译·Time Editor 自身 item view 与 model role 保留英文·QLineEdit 显示翻译不改模型·QTextEdit append 只翻译日志正文·aboutToShow 菜单同步翻译·动态菜单规则化·dylib 预编译·禁止引入目标 Qt 缺失符号·Qt ABI 锁定·ExtensionLayer 自绘层保留英文且不走空补丁
