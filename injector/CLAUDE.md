...
CavalryTranslatorInjector.mm: macOS Objective-C++ runtime 翻译注入器；以 `(context, source)` 哈希复用生成表，在启动期有界补译、交互期只处理 dirty 控件，并以 tooltip 模板、QLabel 父系、QAction owner 与 Tracking 对话框结构为 8 条 exact-only 普通 Qt 文本提供 fail-closed 回补，同时保持菜单首帧、模型 identity、MessageBar 与 session-scoped runtime inventory 的既定边界。
cavalry_i18n_translation_policy.h: 翻译查询边界；跨平台把 CogTool 动态 Pitch、Color Settings/Mesh Explorer 模板、单索引占位文字及 8 条普通 Qt 残留限制在精确 context，并以命名 key 数组向平台 owner-aware 回补与合同测试提供单一真相。
...
法则: 翻译表自动生成·精确哈希保持 first-match-wins·模型 niceName 保留英文·显示层模型名可翻译·已知基名数字后缀仅在基名有嵌入译文时投影·selected-count/认证倒计时保持严格 QLabel；Mesh Explorer、Color Settings 与单索引动态模板还必须命中已采证父系/对话框来源·8 条跨平台普通 Qt 残留禁止进入 source fallback，只允许 exact context 或平台内已采证 owner/控件结构回补·动态 QComboBox 仅改 DisplayRole 且 UserRole/currentIndex 不变·单索引 QPlainTextEdit 仅改精确 placeholderText 且绝不读写正文·无关同文控件保持原文·Scene View 图层列表可翻译·Time Editor 自身 item view 与 model role 保留英文·QLineEdit 显示翻译不改模型·QTextEdit append 只翻译日志正文与有限日志模板·aboutToShow 菜单同步翻译·交互事件禁止全局刷新·普通运行禁止 inventory 写盘·动态菜单规则化·原生库由对应平台生成且不纳入 Git·只加载目标 app 的单套 Qt·禁止引入目标 Qt 缺失符号·Qt ABI 锁定·ExtensionLayer 仅可用共享合同内已验证的 helper/placeholder/MessageBar/text-path 四条精确边界，MessageBar 必须双 caller/末尾 `<br>`/exact body 同时命中且排除 `js_logger`，text-path 必须 exact slot/caller 字节包络/source/context 同时命中且每次 callback 重验；context-only 专用词条禁止进入 source fallback，unknown/纯快捷键 prefix 原样透传；CogTool Pitch 只允许 canonical 32-bit `int` 后缀·三十七项 action/quality/长操作前缀/Bone/Pitch 与 Pencil 三语文案不加末尾句号·CJK 自绘仅可用已锁定 vendor Skia 导出重建白名单 Path，任一失败回退英文·禁止 vendor `.text` / Skia / libc / QPainter 全局 hook·IAT 卸载必须 owner-only 且 restore 失败 fail-open·QString 赋值槽只能由 canonical setter 尾跳解码·禁止 `__cstring` 补丁
