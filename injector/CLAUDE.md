...
成员清单
CavalryTranslatorInjector.mm: Objective-C++ runtime 翻译注入器主源，子类化 QTranslator 拦截 Qt 菜单与普通 QWidget 翻译，阻止 Time Editor 自绘层词汇进入全局模型翻译，规则化处理动态右键菜单标签，支持定时刷新 UI 与 English dump-only 模式输出 runtime inventory。ExtensionLayer `__cstring` 补丁基础设施保留但已禁用（自绘层使用 Latin-only 字体，CJK 显示为 `?`），该层提示保持英文原文。
...
法则: 翻译表自动生成·Time Editor 模型词汇保留英文·动态菜单规则化·dylib 预编译·Qt ABI 锁定·ExtensionLayer 自绘层保留英文
