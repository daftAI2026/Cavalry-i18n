...
### Added
- **Update Availability Surface**: Added a four-language update icon, tooltip, screen-reader announcement, confirmation dialog, and macOS new-bundle ad-hoc/Gatekeeper reminder. The user-supplied arrow renders at 20px inside a 24px circular action only for an explicit local preview or after the Rust updater check reports an available version; artifact signature verification remains part of the later download/install transaction, and this persistent entry intentionally is not duplicated by an update-available Toast.
- **Tauri Updater Foundation**: Added pinned `tauri-plugin-updater` Rust commands for checking and installing updates. Renderer payloads omit artifact URLs, signatures, and raw responses; installation accepts no renderer-controlled artifact input and consumes only the most recently checked `Update` held in Rust State. The final public key and GitHub `latest.json` endpoint are pinned in shared configuration, and a protected no-release smoke has verified both macOS updater architectures. Version 1.0.0 is the manually installed updater bootstrap; a true public-version upgrade can first be exercised when a later SemVer exists.
- **Signed Updater Release Closure**: Added deterministic three-platform `latest.json` generation and tag-only Tauri artifact signing. ReleaseAssetProvenance v5, `SHA256SUMS`, SBOM/toolchain evidence, and private-draft exact readback bind three manual installers plus macOS archives/signatures and the Windows NSIS signature, while explicitly recording macOS ad-hoc, Windows unsigned, and Updater Ed25519 status; ordinary PR/main builds never receive the updater private key.
- **About and Project Links**: Added a localized in-product About dialog with the real Switcher version, the packaged app icon, a GitHub-marked `Cavalry-i18n` project row, and an MIT license link. macOS opens it from the system application menu, Windows exposes one title-bar entry, and the renderer can request only the two Rust-fixed destinations rather than an arbitrary URL.

### Changed
- **Public Documentation Boundary**: Reduced the public `docs/` tree to stable translation rules, repeatable maintenance SOPs, README images, and release badge data. Internal research, roadmaps, event ledgers, live run notes, incident reviews, and superseded plans now live in the adjacent private knowledge repository and are not build, test, or release inputs.
- **Shared Button Primitive and Windows Caption**: Moved every ordinary renderer action onto one local shadcn-source-aligned Button primitive while keeping the Select Trigger in its own combobox state machine. Windows still delegates minimize, maximize/restore, and close behavior to the Tauri system-window API, but renders the three right-side actions as 32px ghost buttons with 4px spacing and 16px Phosphor Regular glyphs.
- **Progressive Installation Selection**: Hide the manual folder action after Cavalry is discovered successfully. It now appears only when no installation was found; every discovered installation remains a read-only summary, and reinstall recovery asks users to reopen the Switcher for automatic detection instead of suggesting that changing folders repairs the same target.
- **Switcher Task Hierarchy and Native Chrome**: Finalized the compact desktop UI around the selected Cavalry installation, one explicit target-language selection, one direct **Switch** flow, and one confirmed **Restore English** action with four-language Activity feedback. The Select now starts with a localized placeholder instead of silently choosing its first option, and Switch stays disabled until the user chooses a target. Switch then enters the fail-before-mutation transaction without a redundant confirmation; a running Cavalry instance is never forced closed, while a closed instance is reopened after the language change. Removed the manual Refresh English action, the separate Recovery section, and the duplicate Restore English/Restore Official entry points. The renderer now uses one semantic token source, dependency-free shadcn/Base UI-aligned Select/Tooltip/Activity state machines, a fixed non-scrolling 400×484 window, and 20px outer padding. macOS keeps native traffic lights and the system About entry; Windows uses a DWM-owned frame with right-side native-semantic caption controls.
- **Metadata and Restore Semantics**: Moved the current-language badge from arbitrary purple to Geist informational `blue-subtle`, while preserving green for verified Official/update meaning and amber/red for warning/error. Restore completion now uses the pinned Phosphor Regular `FloppyDiskBack` glyph to distinguish recovery-baseline writeback from a generic history action.
...
- **About Window Rhythm**: Reused the main window's 40px titlebar, typography, and spacing tokens instead of maintaining a second window geometry. The About content remains optically centered with equal 20px outer padding and an 80px project mark; macOS keeps AppKit traffic-light alignment, while Windows uses the shared transparent compositor, Close action, shadow inset, and platform radius.
- **Release Documentation Truth**: Aligned the SOP, security policy, workflow, and localized READMEs with the implemented ad-hoc macOS tag path. Removed the unused evidence-only commit, offline acceptance attestation, second release-seal key, and protected runner-fingerprint prerequisites; fixed runner labels, full CI, Tauri Updater Ed25519, SHA-256/provenance, SBOM/toolchain evidence, and private-draft byte readback remain the release boundary. Releases that predate the updater closure remain three-installer manual distributions and never gain automatic updates retroactively.

### Fixed
- **Reactive macOS Permission Recovery**: Removed the synthetic first-use App Management gate and its durable “handoff shown” marker. Switch and Restore now enter the same rollback-safe transaction immediately; only a real typed write denial opens the existing System Settings handoff. An isolated new-bundle-id/disposable-Cavalry A/B completed the real transaction without an OS permission prompt, while the reactive fallback remains available for machines that actually deny the write.
- **Quiet macOS Recovery Journal**: Kept crash-safe journal recovery, exact paths, backup hashes, no-follow traversal, preimage/CAS checks, and final signature verification while removing the journal's Keychain HMAC dependency that caused password prompts after ad-hoc rebuilds. Existing schema-6 journals with `authenticationTag` remain readable without another prompt. Switch and Restore now share an early read-only exact-process check, never close a running Cavalry on the user's behalf, and stop before reporting installation/baseline completion.
- **Permission Helper Arrow Geometry**: Expanded the native arrow panel only as transparent stretch/shadow overscan. The 28pt glyph keeps its pre-overscan screen coordinate, so preventing bounce clipping no longer shifts the arrow's initial position.
- **Recovery Copy Clarity**: Rewrote the four-language unsupported-version, reinstall, interrupted-transaction, and running-Cavalry feedback around one state and one recovery path per message. Startup transaction recovery is no longer mislabeled as an updater failure, and Activity titles no longer repeat the same state in their descriptions.
- **Managed Legacy Snapshot Compatibility**: Kept packaged English responsible only for JSON-content proof during p1–p5 macOS migration. The local legacy snapshot and installed Cavalry asset now mutually prove the vendor Unix mode before that mode is bound into the immutable generation, so a historical `755` Cavalry surface is no longer rejected merely because the checked-in language source is an ordinary `644` JSON file. The target-language Select keeps the language already active in Cavalry visible but disabled, and a genuine installation-verification failure now gives a reopen-first, official-reinstall-second recovery path.
- **Legacy macOS Management and Version Safety**: Recognize only exact postimages from the public p1-p5 macOS Switcher releases as Managed Legacy, using the released launcher bytes, injector code identities, language marker, Keychain patch, historical state, and English asset overlay together. The first new write promotes only the proven English JSON into a mode-bound immutable generation, then keeps re-proving that generation without fabricating a vendor baseline. These installations can keep switching languages and restore managed English without being mislabeled as requiring a Cavalry reinstall or as an official vendor restore. Older, newer, and unrecognized Cavalry versions now remain read-only with direction-aware guidance; newer users are never told to downgrade.
- **Desktop-scoped AlertDialog Overlay**: Preserve the titlebar as the stable window identity and native-chrome layer while dimming only the task content below it. Native modal focus, inertness, cancellation, and confirmation semantics remain unchanged.
- **AlertDialog Reading Rhythm**: Kept the approved 16/24 title and 14/20 body roles, moved title-to-body spacing onto the shared 8px rhythm token, and replaced balanced prose wrapping with natural line wrapping so CJK confirmation copy no longer breaks into artificially equalized lines.
...
- **Undo and MessageBar Append-time Contract Tests**: Added comprehensive contract tests in `tools/check_app_contracts.js` to assert bottom status template routing ("Copied", "Undo") for QLabel/QStatusBar text, and localization of dynamic "Undo/Redo" actions across Simplified Chinese, Traditional Chinese, and Japanese.
- **MessageBar Copied Log Template Translation**: Implemented `Copied <object>` dynamic log template translation (`translatedCopiedLogMessage`) inside `CavalryTranslatorInjector.mm` to localize fixed templates (e.g. `已复制「%1」` / `已複製「%1」` / `「%1」をコピーしました`) while dynamically reusing the existing UI object dictionary for target items.
- **MessageBar Copied Log Template Contracts**: Added comprehensive contract tests in `tools/check_app_contracts.js` (`embedded injector translates MessageBar QTextEdit log popout text at append time`) to assert that the dynamic `Copied <object>` templates are correctly recognized, translated, and localized across zh-Hans, zh-Hant, and ja_JP locales.
- **MessageBar QTextEdit Append-Time Translation**: Shifted `MessageBar` log popout translation from the heavy repaint/show hooks (`QEvent::Paint` / `QEvent::Show`) to the dynamic `QTextEdit::append(QString)` interposing interceptor, avoiding redundant document scans (`QTextDocument` block scans) during popup entry animations and preserving macOS native UI performance.
- **QTextEdit::append Dynamic Interposing Interceptor**: Integrated dyld interposing for `QTextEdit::append` (signature `__ZN9QTextEdit6appendERK7QString`) using standard `__DATA,__interpose` runtime interception in `CavalryTranslatorInjector.mm` to dynamically translate log messages in real-time as they are appended to the MessageBar popup window.
- **MessageBar Metadata-Only Diagnostic Inventory**: Exposed meta-object diagnostics (`widgetMetaObjectMethods`, `methodSignature`) for `MessageBar` and `LogMessage` widgets in the injector's runtime inventory, enabling structured live capture mapping while strictly preventing `QTextEdit` history materialization (`toPlainText()`) to maintain dynamic rendering performance.
- **MessageBar & QTextEdit Translation Contracts**: Added contract tests `'embedded injector exposes MessageBar meta-object diagnostics'` and `'embedded injector translates MessageBar QTextEdit log popout text at append time'` in `tools/check_app_contracts.js` to strictly assert meta-object diagnostic capability, public `QTextEdit::append` dynamic interposing, and assert that the runtime refresh/inventory never reads `QTextEdit` history to keep popup animations native.
- **Synchronous Modal Dialog Pre-paint Translation**: Implemented synchronous pre-paint translation for `QDialog`/`QMessageBox` under the `QEvent::Show` event in `CavalryTranslatorInjector.mm` (`translateRuntimeObject`), preventing transient English flashes before the first localized render of modal dialogs and exit confirmation boxes.
- **Modal Dialog Synchronous Translation Contract**: Added a dedicated contract test `'embedded injector translates modal dialogs synchronously before first paint'` in `tools/check_app_contracts.js` to strictly verify that `QDialog` events are intercepted and translated synchronously at pre-paint time via `translateRuntimeObject`, and ensure modal buttons are fully translated through `QDialogButtonBox`.
...
- **Generic QAbstractItemView Model Role Protection in Time Editor**: Added recursive model scanning and normalization (`normalizeTimeEditorModelRows` / `normalizeTimeEditorItemModel`) in the C++ injector (`CavalryTranslatorInjector.mm`) to recursively clean and revert dynamic bracket names back to English for generic `QAbstractItemView` models using `Qt::DisplayRole` and `Qt::EditRole`. This ensures right-side self-drawing tracks in Time Editor that do not use standard `QListWidgetItem`/`QTreeWidgetItem` wrappers are still fully protected against CJK rendering corruptions.
- **Model Role Protection Contract Verification**: Added dedicated contract assertions in `tools/check_app_contracts.js` to verify generic `QAbstractItemView` model roles are normalized strictly within the scoped Time Editor view boundary.
- **QTextEdit::append Symbol Resolution Fallback**: Implemented a dynamic fallback helper (`appendTextEditWithoutInterpose`) in the injector (`CavalryTranslatorInjector.mm`) that directly appends text/HTML logs using a `QTextCursor` if `dyld` fails to resolve the next `QTextEdit::append` symbol. This prevents log messages from being swallowed or showing blank popup windows in the MessageBar.
- **QTextEdit::append Safe Fallback Verification**: Added contract assertions in `tools/check_app_contracts.js` to ensure the `QTextEdit::append` interposing interceptor does not silently discard log text upon symbol resolution failure and implements the cursor-based write-back path.

### Changed
- **GEB Document Mapping Sync for Undo & MessageBar Performance**: Updated L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to document the new `Undo/Redo` template matching and append-time log translation interface.
- **Recompiled Injector Dylib with Append-Time Log Translation**: Rebuilt and updated the precompiled library `injector/libCavalryTranslatorInjector.dylib` to embed the append-time QTextEdit log translator and unified QLabel/QStatusBar status routing, keeping rendering pipelines pure.
- **Runtime Injection Documentation Sync**: Updated runtime injection documentation (`docs/cavalry-runtime-injection-techniques.md`) to reflect the removal of pre-paint QTextDocument revision scanning and the focus on append-time interpose injection.
- **GEB Document Mapping for Copied Log Templates**: Updated L1 `docs/CLAUDE.md`, L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to reflect the MessageBar `Copied` dynamic log template translation.
- **Recompiled Injector Dylib with Copied Log Translation**: Recompiled and updated the universal dynamic library `injector/libCavalryTranslatorInjector.dylib` to embed the `Copied <object>` dynamic log template translation engine.
- **GEB Document Mapping for QTextEdit and MessageBar**: Updated L1 `docs/CLAUDE.md`, L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to reflect the new `QTextEdit` append-time log translation, metadata-only diagnostics, and strict `QTextEdit` plainText materialization blocks.
- **Recompiled Injector Dylib**: Recompiled and updated the universal dynamic library `injector/libCavalryTranslatorInjector.dylib` to embed the `QTextEdit` append-time interposing interceptor, metadata-only diagnostics, and plainText reading locks.
- **Runtime Capture and Injection Documentation Sync**: Updated runtime capture and injection documentation (`docs/runtime-ui-live-capture-workflow.md` and `docs/cavalry-runtime-injection-techniques.md`) to outline the append-time log translation sequence, metadata-only inventory constraints, and widget event mapping schemas for QTextEdit/QLineEdit/QDialog/QMenu.
- **GEB Document Mapping for Modal Translation**: Updated L2 module maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and updated L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to reflect the newly integrated `QDialog` pre-paint synchronous translation and its associated contract test.
- **Apply Character Spacing English Data Layer Protection**: Reverted the attributes `pairs`, `pairs.matchString`, and `pairs.spacing` inside the language JSON packs (`languages/*/nodeStrings.json`) back to English (`Matches`, `Match String`, `Character Spacing`), and placed them back under `no_translate` in `tools/translation-whitelist.json`. This protects the underlying Time Editor data-layer from CJK character corruptions and Latin-only Canvas self-drawing rendering boxes, while allowing display-level translation to be handled by the compiled UI TS catalogs and the dynamic Qt injector.
- **GEB Document Mapping Sync for Model Roles**: Updated L2 module maps in `injector/CLAUDE.md`, `languages/CLAUDE.md`, and `tools/CLAUDE.md` to document the new `QLineEdit` repaint interceptor, the dynamic bracketed name translation / Time Editor English restoration protocol, and the generic `QAbstractItemView` model role protection with its validation contracts.
- **GEB Document Mapping Sync for Safe Fallback**: Updated L2 module maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and updated L3 headers in `CavalryTranslatorInjector.mm` and `tools/check_app_contracts.js` to document the new `QTextEdit::append` fallback write path.
- **Recompiled Injector Dylib**: Recompiled and updated the universal dynamic library `injector/libCavalryTranslatorInjector.dylib` to embed the new dynamic `QTextEdit::append` safe fallback write path.
...
- **Composition Menu Lazy QAction Flicker Audit**: Added the dated Composition menu audit documenting dynamic QAction show-time resolution, comparing Qt inventory state with AppKit native menu AX sampling, correcting the "delay-translation" design flaw, and detailing the synchronous `aboutToShow` pre-paint translation fix.
