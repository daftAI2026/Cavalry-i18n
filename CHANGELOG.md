...
### Added
- **Update Availability Surface**: Added a four-language update icon, tooltip, screen-reader announcement, confirmation dialog, and macOS new-bundle ad-hoc/Gatekeeper reminder. The user-supplied arrow renders at 18px inside a 24px circular action only for an explicit local preview or a signed updater result.
...
- **About and Project Links**: Added a localized in-product About dialog with the real Switcher version, full GitHub project address, and MIT license link. macOS opens it from the system application menu, Windows exposes one title-bar entry, and the renderer can request only the two Rust-fixed destinations rather than an arbitrary URL.

### Changed
- **Switcher Task Hierarchy and Native Chrome**: Finalized the compact desktop UI around the selected Cavalry installation, one primary language action, secondary recovery actions, and status feedback with result-specific four-language titles. The renderer now uses one semantic token source, a shadcn/Base UI-aligned dependency-free Select state machine, a 460×404 content closure, 20px outer padding, and divider-free 32px section rhythm. macOS keeps native traffic lights and the system About entry; Windows uses a DWM-owned frame with right-side native-semantic caption controls.
