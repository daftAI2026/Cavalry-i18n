...
- **QTextDocument Revision Cache**: Introduced a rendering performance optimization in `translateTextEditDocument` using `QTextDocument::revision()` to avoid scanning the entire document block-by-block unless the text content has actually changed, preventing CPU overhead during dynamic popup entry animations (multiple continuous repaint events).
- **Undo and Performance Optimization Contract Tests**: Added comprehensive contract tests in `tools/check_app_contracts.js` to assert bottom status template routing ("Copied", "Undo") for QLabel/QStatusBar text, QTextDocument revision-based repaint caching, and localization of dynamic "Undo/Redo" actions across Simplified Chinese, Traditional Chinese, and Japanese.
...
- **MessageBar Copied Log Template Contracts**: Added comprehensive contract tests in `tools/check_app_contracts.js` (`embedded injector translates MessageBar QTextEdit log popout text before paint`) to assert that the dynamic `Copied <object>` templates are correctly recognized, translated, and localized across zh-Hans, zh-Hant, and ja_JP locales.
- **QTextEdit Log Popout Pre-paint Translation**: Implemented synchronous pre-paint translation for `QTextEdit` widget documents (`translateTextEditDocument`) in `CavalryTranslatorInjector.mm` to recursively translate appended text blocks before the first visible render. Checks `textEditForObject` to resolve sub-viewport paint events back to the parent `QTextEdit` container.
...
- **MessageBar Meta-Object Diagnostic Inventory**: Exposed meta-object diagnostics (`widgetMetaObjectMethods`, `methodSignature`) for `MessageBar` and `LogMessage` widgets in the injector's runtime inventory, enabling structured live capture mapping without depending on visual coordinates.
- **MessageBar & QTextEdit Translation Contracts**: Added contract tests `'embedded injector exposes MessageBar meta-object diagnostics'` and `'embedded injector translates MessageBar QTextEdit log popout text before paint'` in `tools/check_app_contracts.js` to strictly assert meta-object diagnostic capability, event mapping, and public `QTextEdit::append` dynamic interposing.
...
### Changed
- **GEB Document Mapping Sync for Undo & Performance Cache**: Updated L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to document the new `Undo/Redo` template matching, `QTextDocument` revision-based paint de-duplication, and generic widget text routing.
- **Recompiled Injector Dylib with Performance Cache & Undo Translation**: Rebuilt and updated the precompiled library `injector/libCavalryTranslatorInjector.dylib` to embed the performance-optimized QTextDocument cache, dynamic Undo/Redo matcher, and unified QLabel/QStatusBar status routing.
- **Runtime Injection Documentation Sync**: Updated runtime injection documentation (`docs/cavalry-runtime-injection-techniques.md`) to detail pre-paint performance boundaries, `QTextDocument` revision caching, `QStatusBar` text mapping, and `Undo/Redo` template decomposition workflows.
...
- **GEB Document Mapping for QTextEdit and MessageBar**: Updated L1 `docs/CLAUDE.md`, L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to reflect the new `QTextEdit` and `MessageBar` pre-paint synchronous translation and meta-object diagnostic contracts.
- **Recompiled Injector Dylib**: Recompiled and updated the universal dynamic library `injector/libCavalryTranslatorInjector.dylib` to embed the `QTextEdit` pre-paint document translation, `append` interposing interceptor, and meta-object diagnostics engine.
- **Runtime Capture and Injection Documentation Sync**: Updated runtime capture and injection documentation (`docs/runtime-ui-live-capture-workflow.md` and `docs/cavalry-runtime-injection-techniques.md`) to outline the pre-paint synchronous translation sequence and widget event mapping schemas for QTextEdit/QLineEdit/QDialog/QMenu.
