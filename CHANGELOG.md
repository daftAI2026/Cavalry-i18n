...
### Added
- **Update Availability Surface**: Added a four-language update icon, tooltip, screen-reader announcement, confirmation dialog, and macOS new-bundle ad-hoc/Gatekeeper reminder. The user-supplied arrow renders at 20px inside a 24px circular action only for an explicit local preview or a signed updater result; this persistent entry intentionally is not duplicated by an update-available Toast.
- **Tauri Updater Foundation**: Added pinned `tauri-plugin-updater` Rust commands for checking and installing updates. Renderer payloads omit artifact URLs, signatures, and raw responses; installation accepts no renderer-controlled artifact input and consumes only the most recently checked `Update` held in Rust State. The final public key and GitHub `latest.json` endpoint are pinned in shared configuration; a protected no-release smoke has verified both macOS updater architectures, while the current candidate package, Windows signing, and cross-version live evidence remain required before automatic updates can ship.
