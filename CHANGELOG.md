...
### Changed
- **Progressive Installation Selection**: Hide the manual folder action after Cavalry is discovered successfully. It now appears only when no installation was found or choosing another installation is a real recovery path, so the healthy installation card remains a read-only summary without trapping reinstall or unwritable custom-root failures.
