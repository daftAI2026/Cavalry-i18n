...
- **Tauri Updater Foundation**: Added pinned `tauri-plugin-updater` Rust commands for checking and installing updates. Renderer payloads omit artifact URLs, signatures, and raw responses; installation accepts no renderer-controlled artifact input and consumes only the most recently checked `Update` held in Rust State. The final public key and GitHub `latest.json` endpoint are pinned in shared configuration; a protected no-release smoke has verified both macOS updater architectures, while current-HEAD packaging, Windows updater/live, and cross-version evidence remain required before automatic updates can ship.
- **Signed Updater Release Closure**: Added deterministic three-platform `latest.json` generation and tag-only Tauri artifact signing. The schema v6 release seal, schema v4 provenance, `SHA256SUMS`, and private-draft exact readback now bind three manual installers plus macOS archives/signatures and the Windows NSIS signature, while explicitly recording `macos: ad-hoc`; ordinary PR/main builds never receive the updater private key.
...
- **About Window Rhythm**: Reused the main window's 40px Overlay titlebar, AppKit traffic-light alignment, typography, and spacing tokens in the macOS About window instead of maintaining a second native-titlebar geometry. The About content remains optically centered with equal 20px outer padding, and its project mark increases by one 4px design step from 64px to 68px; Windows keeps its native caption until that platform is verified.
- **Release Documentation Truth**: Aligned the SOP, release-seal contract, security policy, roadmap, and localized READMEs with the implemented ad-hoc macOS tag path. Releases that predate the updater closure remain three-installer manual distributions; updater artifacts, schema v6/v4 sidecars, and automatic-update availability begin only with a successfully verified updater-enabled tag and never apply retroactively.
