...
- **GEB Document Mapping for QTextEdit and MessageBar**: Updated L1 `docs/CLAUDE.md`, L2 maps in `injector/CLAUDE.md` and `tools/CLAUDE.md`, and L3 headers in `CavalryTranslatorInjector.mm` and `check_app_contracts.js` to reflect the new `QTextEdit` append-time log translation and meta-object diagnostic contracts.
- **Recompiled Injector Dylib**: Recompiled and updated the universal dynamic library `injector/libCavalryTranslatorInjector.dylib` to embed the `QTextEdit` append-time interposing interceptor and meta-object diagnostics engine.
- **Runtime Capture and Injection Documentation Sync**: Updated runtime capture and injection documentation (`docs/runtime-ui-live-capture-workflow.md` and `docs/cavalry-runtime-injection-techniques.md`) to outline the append-time log translation sequence and widget event mapping schemas for QTextEdit/QLineEdit/QDialog/QMenu.
