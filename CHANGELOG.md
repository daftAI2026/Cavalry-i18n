...
- 修复 macOS 正常重新签名后，被误判为翻译运行库已遭修改、导致无法更新或切换语言的问题；真实代码与权限变化仍会阻止覆盖。
- 选择带有“可更新”徽章的语言后，收起下拉菜单仍显示该徽章，并随补丁状态刷新。
- 修复 macOS 和 Windows 时间编辑器中的中文、日文图层名称显示为方框；字体回退按名称实际字符选择，支持不同界面语言下的混合名称。
...
### Added
- Added update notifications and signed in-app updates for future Switcher versions.
- Added a localized About window with project and license links.

### Changed
- Redesigned the Switcher around one language selector, one **Switch** action, and one **Restore English** action.
- Added clearer task progress, warnings, permission guidance, and platform-native window controls.
- Improved Cavalry discovery and kept the current language visible but unavailable for redundant selection.

### Fixed
- Ask for macOS App Management permission only after a real write denial instead of interrupting every first run.
- Removed the recovery journal Keychain prompt caused by ad-hoc rebuilds.
- Restored compatibility with Cavalry installations translated by earlier p1–p5 Switcher releases.
- Fixed About-window lifecycle, long localized dialog wrapping, update progress, and activity scrolling.
