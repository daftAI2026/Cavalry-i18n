| en | zh-Hans | zh-Hant | ja_JP |
|---|---|---|---|
| Language Switcher | 语言切换器 | 語言切換器 | 言語切り替え |
| Position | 位置 | 位置 | 位置 |
| Rotation | 旋转 | 旋轉 | 回転 |
| Scale | 缩放 | 縮放 | スケール |
| Opacity | 不透明度 | 不透明度 | 不透明度 |
| Transform | 变换 | 變換 | トランスフォーム |
| Anchor Point | 锚点 | 錨點 | アンカーポイント |
| Pivot | 锚点 | 錨點 | ピボット |
| Keyframe | 关键帧 | 關鍵幀 | キーフレーム |
| Easing | 缓动 | 緩動 | イージング |
| Ease In | 缓入 | 緩入 | イーズイン |
| Ease Out | 缓出 | 緩出 | イーズアウト |
| Composition | 合成 | 合成 | コンポジション |
| Pre-compose | 预合成 | 預合成 | プリコンポーズ |
| Layer | 图层 | 圖層 | レイヤー |
| Parent | 父级 | 父級 | 親 |
| Child | 子级 | 子級 | 子 |
| Mask | 蒙版 | 遮罩 | マスク |
| Stroke | 描边 | 描邊 | ストローク |
| Fill | 填充 | 填充 | 塗り |
| Bezier | 贝塞尔 | 貝茲 | ベジェ |
| Path | 路径 | 路徑 | パス |
| Curve | 曲线 | 曲線 | カーブ |
| Viewport | 视口 | 檢視區 | ビューポート |
| Deformer | 变形器 | 變形器 | デフォーマ |
| Shader | 着色器 | 著色器 | シェーダー |
| Render | 渲染 | 算繪 | レンダリング |
| Blending Mode | 混合模式 | 混合模式 | 描画モード |
| Expression | 表达式 | 運算式 | エクスプレッション |
| Null | 空对象 | 空物件 | ヌル |
| Duplicator | 复制器 | 複製器 | デュプリケーター |
| Node | 节点 | 節點 | ノード |
| Basic Shape | 基本形状 | 基本形狀 | 基本シェイプ |
| Property | 属性 | 屬性 | プロパティ |
| Tooltip | 工具提示 | 工具提示 | ツールチップ |
| Tip | 提示 | 提示 | ヒント |
| Onboarding | 新手引导 | 新手導覽 | オンボーディング |
| Plugin | 插件 | 外掛程式 | プラグイン |
| Preset | 预设 | 預設 | プリセット |
| Timeline | 时间线 | 時間軸 | タイムライン |
| Graph Editor | 图形编辑器 | 圖形編輯器 | グラフエディター |
| Motion Blur | 运动模糊 | 動態模糊 | モーションブラー |
| Gradient | 渐变 | 漸層 | グラデーション |
| Scene | 场景 | 場景 | シーン |
| Preview | 预览 | 預覽 | プレビュー |
| Playback | 播放 | 播放 | 再生 |
| Loop | 循环 | 循環 | ループ |
| Resolution | 分辨率 | 解析度 | 解像度 |
| Frame Rate | 帧率 | 幀率 | フレームレート |
| Workspace | 工作区 | 工作區 | ワークスペース |
| Preferences | 首选项 | 偏好設定 | 環境設定 |
| Apply | 应用 | 套用 | 適用 |
| Restart | 重新启动 | 重新啟動 | 再起動 |
| Apply & Restart | 应用并重新启动 | 套用並重新啟動 | 適用して再起動 |
| Save | 保存 | 儲存 | 保存 |
| Save As | 另存为 | 另存新檔 | 名前を付けて保存 |
| Open | 打开 | 開啟 | 開く |
| Close | 关闭 | 關閉 | 閉じる |
| File | 文件 | 檔案 | ファイル |
| Edit | 编辑 | 編輯 | 編集 |
| View | 视图 | 檢視 | 表示 |
| Window | 窗口 | 視窗 | ウィンドウ |
| Scripts | 脚本 | 腳本 | スクリプト |
| Alpha | Alpha | Alpha | Alpha |
| RGB | RGB | RGB | RGB |
| CMYK | CMYK | CMYK | CMYK |
| SVG | SVG | SVG | SVG |
| JSON | JSON | JSON | JSON |
| CSV | CSV | CSV | CSV |
| FPS | FPS | FPS | FPS |
| BPM | BPM | BPM | BPM |
| GPU | GPU | GPU | GPU |
| Lottie | Lottie | Lottie | Lottie |
| Cavalry | Cavalry | Cavalry | Cavalry |
| Canva | Canva | Canva | Canva |
| Forge Dynamics | Forge Dynamics | Forge Dynamics | Forge Dynamics |
| Undo | 撤销 | 復原 | 元に戻す |
| Redo | 重做 | 重做 | やり直し |
| Copy | 复制 | 複製 | コピー |
| Paste | 粘贴 | 貼上 | ペースト |
| Delete | 删除 | 刪除 | 削除 |
| Group | 编组 | 群組 | グループ |
| Export | 导出 | 匯出 | エクスポート |
| Import | 导入 | 匯入 | インポート |
| Color | 颜色 | 顏色 | カラー |
| Animation | 动画 | 動畫 | アニメーション |
| Offset | 偏移 | 偏移 | オフセット |
| Select All | 全选 | 全選 | すべて選択 |
| Default | 默认 | 預設 | デフォルト |
| Video | 视频 | 影片 | ビデオ |
| Program | 程序 | 程式 | プログラム |
| Information | 信息 | 資訊 | 情報 |
| Software | 软件 | 軟體 | ソフトウェア |
| Print | 打印 | 列印 | 印刷 |
