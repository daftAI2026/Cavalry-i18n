...
- 注入路径默认走 `injector 生产链路：`codesign --remove-signature` → `codesign --force --deep --sign -` ad-hoc 重签 → `DYLD_INSERT_LIBRARIES=$INJECTOR_PATH` 启动 Cavalry。该链路在 SIP=enabled 的机器上长期工作，是默认 G-CAPTURE 真相路径
