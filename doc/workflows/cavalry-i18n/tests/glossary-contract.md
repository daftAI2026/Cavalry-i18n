<!--
[INPUT]: 依赖 doc/cavalry-glossary-en-zh.md（初始术语表）、doc/translation-guidelines.md（翻译原则）
...
验证 `doc/cavalry-glossary.md` 已扩展为四语言术语表（en / zh-Hans / zh-Hant / ja_JP），数据完整、简繁差异正确、不翻译项保持英文。

## Behaviors

### B1: 文件存在且表头包含四列语言

RED：`doc/cavalry-glossary.md` 不存在或表头缺列。

```bash
FILE="doc/cavalry-glossary.md"
...
```bash
DATA_LINES=$(grep "^|" "doc/cavalry-glossary.md" | grep -v "^|--" | grep -v "^| en" | wc -l | tr -d ' ')
...
```bash
EMPTY=$(grep -E '\|[[:space:]]*\|' "doc/cavalry-glossary.md" | grep -v "^|--" | wc -l | tr -d ' ')
if [ "$EMPTY" -gt 0 ]; then
