...
验证 `doc/translation-whitelist.json` 存在、合法、覆盖所有文件类型，每个类型有 `translate` + `no_translate` 列表，且 translate 字段在实际 JSON 中存在。

## Behaviors

### B1: 白名单文件存在且合法 JSON

RED：`doc/translation-whitelist.json` 不存在或不可解析。

```bash
WHITELIST="doc/translation-whitelist.json"
if [ ! -f "$WHITELIST" ]; then
...
wl = json.load(open('doc/translation-whitelist.json'))
