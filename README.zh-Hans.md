...
[OUTPUT]: 对外提供 macOS / Windows 用户安装、当前补丁更新、搜索排序限制、开发与安全说明的简体中文版本
...
**Add Layer 搜索：** 可以使用英文或当前语言搜索。右侧 Add Layer 面板中，本地语言的匹配结果（包括说明文字匹配）不一定获得与英文名称相同的标题优先级。如果目标图层排得较后，可以尝试完整英文名称，例如 `Text Shape`。这是已知的排序限制，不代表搜索输入或图层的创建身份被改写。
