...
[OUTPUT]: 对外提供 macOS / Windows 用户安装、当前补丁更新、搜索排序限制、开发与安全说明的日本語版
...
**Add Layer の検索：** 英語と選択中の言語で検索できます。右側の Add Layer パネルでは、翻訳された名前や説明文への一致が、英語名への一致と同じタイトル優先度にならない場合があります。目的のレイヤーが下位に表示される場合は、`Text Shape` などの完全な英語名を試してください。これは既知の並び順の制限であり、検索入力やレイヤー作成時の識別情報が書き換わったことを意味しません。
