...
macos_permission_handoff_contract.rs: 只读断言 App Management handoff 仍在九命令内，固定 permission/source rect/CSS viewport/per-session Channel 合同由真实 apply 结果收口；native owner 只用公开 nonactivating panel/file-URL drag/Reduce Motion API，并仅在 Copy 落点属于实时 System Settings 整窗时请求重试，禁止 TCC/AX/合成输入自动授权。
