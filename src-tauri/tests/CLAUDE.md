...
manual_windows_live_smoke.rs: 默认 ignored 的 Windows disposable live-clone 证据门；可用语言过滤器跑单语或默认三语，子进程隔离 AppData，自动捕获 Viewport Quality/Transform 与 exact-HWND `A` 键触发的 Edit Shape；显式 `CAVALRY_I18N_WINDOWS_LIVE_COG_PITCH=1` 时先拒绝预置 bit 15，追加人工 Cogwheel 拖拽，并要求前后 revision/canonical/whitelist/CJK-success 严格增长、零 fallback/renderer failure 与 PNG，outstanding PID 只优雅关闭 owned clone，unwind 仍恢复 English 38 JSON，禁止场景脚本、Qt UIA 与坐标假 gate。
