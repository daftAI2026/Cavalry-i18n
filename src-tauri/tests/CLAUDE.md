...
bridge_webview_contract.rs: 通过 Node 执行 `bridge::script()` 返回的实际 Rust initialization include，断言冻结 `window.cavalryI18n`、camelCase/warningCodes、强类型有序 Channel 的 phase 投影与 Builder/HTML source 顺序；明确不替代 packaged WebView/CSP 外部门。
