...
manual_windows_live_smoke.rs: 默认 ignored 的 Windows disposable live-clone 证据门；可用语言过滤器跑单语或默认三语 QPA 原生启动，子进程隔离 AppData，自动捕获 Viewport Quality/Transform 并要求 Transform action 与三条长操作前缀完整位图，再经有界 exact-HWND 前台门以 `A` 键触发 Edit Shape，要求六条 action 与三条长操作前缀完整位图；`Space` 等纯键位不计翻译命中，显式 Cog Pitch 模式要求 bit 28 与计数严格增长、零 fallback，outstanding PID 只优雅关闭 owned clone，unwind 以显式 English 恢复资源，禁止场景脚本、Qt UIA 与坐标假 gate。
