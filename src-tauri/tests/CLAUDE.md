...
macos_permission_handoff_contract.rs: 只读断言 App Management handoff 仍在九命令内，固定 permission/source rect/CSS viewport/per-session Channel 合同由真实 apply 结果收口；drop 后只运行一次同进程 oracle，任何失败均 cleanup，权限链不复制 macOS 的“退出并重新打开”，程序 self-restart 只归 updater 安装完成路径；native owner 只用公开 nonactivating panel/file-URL drag/Reduce Motion API，532×112 helper 固定“单行 Drag 指令 / Back + App row”几何，整条 App row 快照排除兄弟 NSBox 背景，motion surface 与 shadow/stroke 共用单一几何 owner，箭头以 reference spring、bottom anchor 和 overscan canvas 防止 stretch/shadow 裁切，并仅在 Copy 落点属于实时 System Settings 整窗时请求重试，禁止 TCC/AX/合成输入自动授权。
...
patch_contract.rs: 断言 English 提取、精确插件 manifest/hash、重复 canonical destination/component-boundary fail closed、原始 Unix mode manifest 与 mac exact restore/overlay pairs、copy pair/snapshot、packaged-English 逐叶内容证明与 revision provenance 失效；Managed Legacy 迁移明确让 packaged English 只证明内容、让 legacy snapshot 与 installed asset 互证并绑定本机 mode；同时验证 keyed overlay 保留 smoother/未来节点，并锁定 smoother 属性的英简繁日四语同构。
