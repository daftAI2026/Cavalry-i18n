...
bridge_webview_contract.rs: 通过 Node 执行 `bridge::script()` 返回的实际 Rust initialization include，断言冻结 `window.cavalryI18n`、camelCase/warningCodes、Apply 四阶段与 Updater 三阶段强类型有序 Channel 的脱敏投影，以及 Builder/HTML 中 operation-log→update-progress→app 的 source 顺序；明确不替代 packaged WebView/CSP 外部门。
