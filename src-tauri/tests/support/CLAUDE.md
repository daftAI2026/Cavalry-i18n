...
成员清单
windows_disposable.rs: Windows ignored smoke 的共享路径安全边界；只接受显式环境变量指向的 `%TEMP%` 严格子目录和 disposable sentinel，把 drive/verbatim/8.3 拼写统一为规范路径身份，并对安装根、证据根、JSON/plugin/marker 及 QPA root/recovery/固定临时目标执行 canonical containment 与逐级 reparse 拒绝；另以固定 magic sentinel 排他准备/清理仅供 Onboarding/Adjacent 使用的 Local/Roaming `qttest/Cavalry`，不触碰 FullSurfaces 的真实 profile。
windows_workspace_guard.rs: FullSurfaces 的用户档案安全边界；启动前检查 clone 的 `assets/Icons/sign-in-bg.png`、`cavByCanva.png`、`tool_search.png` 为非空普通文件并把字节哈希写入 evidence，运行前将真实 `%LOCALAPPDATA%\Cavalry\workspace.json` 预镜像到 evidence，进程清零后以同卷临时文件精确恢复存在性/字节/只读属性，再做最终字节证明。
...
windows_live_orchestration.inc.rs: live-clone 事务编排分片；管理语言 apply、仅 Onboarding/Adjacent 使用的 Qt test profile、acceptance-only plugin 临时安装、exact-PID/HWND 进程清理、English 字节恢复与真实 workspace 精确恢复，并把 `tools/macos-acceptance/fixtures` 的两枚最小 PNG 作为双平台 Assets producer 输入冻结到每语唯一 stem。
windows_live_tests.inc.rs: Windows live 门入口分片；冻结 helper/driver 禁用原语、Qt profile/Next 转场/清理顺序，在每个 disposable gate 前后调用 workspace/clone guard 证明真实 `%LOCALAPPDATA%/Cavalry/workspace.json` 已按预镜像恢复，并暴露 full-surface、Onboarding、Adjacent 三个 ignored 人工像素复核门；显式 release 环境下只在清理成功后写入 machine record/inventory，绝不写人工 PASS。
