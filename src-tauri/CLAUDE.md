...
tauri.conf.json: Tauri 公共配置，以显式 `./index.html` 进入 renderer、关闭 `withGlobalTauri`、设置本地 CSP，固定 updater 公钥与 GitHub `latest.json` endpoint；主窗默认 460×428，macOS 使用 decorations + Overlay + hiddenTitle 保留系统原生交通灯，renderer 只承担拖拽区与标题内容。
...
capabilities/: Tauri v2 capability 配置，限定 main window 的 core 权限，并只额外开放 renderer 标记标题区所需的 `start_dragging`。
...
tests/: Rust contract tests，守住展示版本/内容 revision 分离、八命令 DTO、脱敏 Updater 状态、updater 内嵌公钥与显式候选流式验签、clean-English 采集、未知 JSON 数据保留、Windows restart，以及 macOS 真实冒烟和 Windows disposable clone 的非 GUI/基础截图加逐类人工证据双现场门。
