...
成员清单
Cargo.toml: Rust crate 与 Tauri v2 依赖声明，`tauri`/`tauri-build` 保持精确版本线，`libc` 仅承载 macOS advisory `flock` 边界。
Cargo.lock: Rust 依赖锁定文件，冻结 Tauri、serde、chrono、libc 等后端依赖版本。
...
src/: Rust command 与纯函数模块，承载桌面业务能力、跨进程 bundle 单飞编排和 Tauri-only 权限/签名入口。
tests/: Rust contract tests，守住版本、配置、command JSON shape、窗口 bridge、owned Keychain 补丁、增量签名验证/修复、文件映射与真实 macOS 手动冒烟入口。
