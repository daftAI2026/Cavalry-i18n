...
tauri.macos.conf.json: macOS 合并配置，在 dev/build 前生成 injector，独占 dylib/languages 资源、DMG 与 ad-hoc signing。
tauri.windows.conf.json: Windows 合并配置，在 dev 前构建 generic translator + QPA delegate，在 bundle 前构建双 DLL 并准备 NSIS provenance，绑定仅清理安装元数据的卸载收尾 hook，独占自动跟随系统语言的四语 NSIS、品牌 ico、languages、`cavalryi18n.dll` 与 `qwindows.dll` 资源；禁止携带 macOS dylib。
