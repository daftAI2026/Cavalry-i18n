...
tauri.conf.json: Tauri 公共配置，以显式 `./index.html` 进入 renderer、关闭 `withGlobalTauri`、设置本地 CSP，固定 updater 公钥与 GitHub `latest.json` endpoint，并注册 `main`/`about` 两个 capability；主窗固定 400×480，内容宽 360px 由 20px 双侧留白推导，两个动作轨道为 170px + 20px + 170px，macOS 使用 decorations + Overlay + hiddenTitle 保留系统原生交通灯，renderer 只承担拖拽区与标题内容。
...
tauri.windows.conf.json: Windows 合并配置，以完整 main-window override 将系统 caption 关闭、保留 DWM shadow，并维持共享 400×480 固定最小几何与 360px 内容宽度；在 dev/bundle 前构建 generic + QPA，绑定四语双语义卸载 hook 与 NSIS 消息表，独占 ico/languages/双 DLL，禁止携带 macOS dylib。
...
capabilities/: Tauri v2 capability 配置，限定 main window 的 core 权限，并为独立 About window 单独开放版本读取；About renderer 不获得任意窗口位置、尺寸、装饰或窗口管理权限。
...
tests/: Rust contract tests，守住展示版本/内容 revision 分离、九命令 DTO/固定项目外链、`main`/`about` 能力边界、脱敏 Updater 状态、updater 内嵌公钥与显式候选流式验签、clean-English 采集、未知 JSON 数据保留、Windows restart，以及 macOS 真实冒烟和 Windows disposable clone 的非 GUI/基础截图加逐类人工证据双现场门。
