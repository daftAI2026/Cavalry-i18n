...
tauri.macos.conf.json: macOS 合并配置，在 dev/build 前生成 injector，独占 dylib/languages 资源与 DMG；通过 schema 支持的 `bundle.macOS.files` 把四个 `InfoPlist.strings` 映射到 `Contents/Resources/*.lproj`；不硬编码 signing identity，本地 ad-hoc 与 tag Developer ID 由显式环境分流。
