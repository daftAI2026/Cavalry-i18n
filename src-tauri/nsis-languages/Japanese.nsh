; [INPUT]: 依赖 Tauri NSIS 模板的 PRODUCTNAME、VERSION 与 Japanese 语言常量
; [OUTPUT]: 对外提供日语安装/卸载消息，并在确认页限定应用数据为切换器设置
; [POS]: nsis-languages 的日语消息表；键集合与 Tauri 2.10.3 默认表同构，只收窄 deleteAppData 的产品语义
