 ...
 * [OUTPUT]: 覆盖 direct 事务回滚、typed cleanup warning、Windows apply 退出码与用户动作内 Program Files recovery same-EXE launcher 边界。
...
#[cfg(target_os = "windows")]
#[test]
fn program_files_action_recovery_uses_the_same_exe_recovery_worker() {
    use std::cell::Cell;
