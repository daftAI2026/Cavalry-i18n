...
journal_manifest.rs: 版本化 `journal.state`/`.tmp` 崩溃恢复真相；manifest 以 no-share + OPEN_REPARSE_POINT 句柄读取/写入并严格验证每个 entry 的路径、pre/postimage、backup、权限与 phase；state 是已发布权威代，state.tmp 是提交候选，双代分歧时保守采用 state，state 缺失才接受完整 tmp，并决定下次启动的 commit/rollback/cleanup。
