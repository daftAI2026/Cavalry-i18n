...
journal_manifest.rs: schema v3 版本化 `journal.state`/`.tmp` 崩溃恢复真相；manifest 以 no-share + OPEN_REPARSE_POINT 句柄读取/写入并严格验证每个 entry 的路径、pre/postimage、backup、权限、双向 displaced intent 与 phase；state 是已发布权威代，state.tmp 是提交候选，双代分歧时保守采用 state，state 缺失才接受完整 tmp，并决定下一次用户语言动作的 commit/rollback/cleanup。
...
worker.rs: 同一 EXE 的 apply/recovery 提权 headless worker；重解 Known Folder/固定目标，QPA 首写前落盘全部精确中间态，并在下一次用户语言动作中复用 durable recovery，绝不写 state、重启或获取外层 operation lock。
...
原子 payload 约束：DurableJournal prepare 先在不含 JOURNAL_PREFIX 的 preparation root 中完成 preimage、manifest 与 fsync，再以同目录原子 rename 发布；下一次用户语言动作只发现已发布 journal。storage.rs 只通过 destination_io 的固定 `.payload-{apply|rollback}-{entry}.tmp` 与 `.payload-displaced-{entry}.tmp` journal 成员进行 staged 写入；临时内容、权限、哈希和目录必须先持久化，既有目标用 ReplaceFileW 捕获发布瞬间 displaced 前像，缺失目标用不覆盖式 MoveFileExW 原子发布，并以 no-share 句柄复核。action-time inspector 必须允许且只允许这些声明成员；未知、重解析或漂移临时对象保持 fail-closed，journal 自有 readonly 成员仅在精确枚举与 handle-bound 打开后清除属性再删除。
