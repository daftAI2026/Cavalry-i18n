...
parent.rs: 非提权父进程编排；在旧 close/copy 前识别 OS-known Program Files，严格 staging 固定 payload；全量目标与 QPA Noop 只读证明一致时零 UAC 提交，否则只调用一次 launcher，并把 45 投影为可重试 StillRunning，已启动后结果未知则保留绑定 staging。
...
parent_tests.rs: 父进程隔离合同；覆盖 NotApplicable、取消、零 UAC Noop、启动前清理、启动后 staging 保留、0/42/43/44/45/未知、严格映射与连续 marker preimage。
