...
macos/: macOS durable apply transaction、bundle 维护与 exact-PID 适配器；承担 fd-bound copy/restore、首次 launch gate、codesign、quarantine 与 Privacy & Security 入口，其中签名证据解析同时覆盖 vendor requirement 与 `codesign` 为 ad-hoc 默认 requirement 输出的注释前缀。
