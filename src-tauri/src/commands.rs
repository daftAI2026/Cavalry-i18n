 ...
 * [OUTPUT]: 保持六条稳定 Tauri command、apply/extract 兼容入口；extract 只生成 English snapshot 并返回 Windows residue 的 typed reconciliationRequired 检测结果，renderer 的 English restore 直接复用既有 apply transaction；get_status 从安装现实重算该 typed 状态并显式投影启动恢复阻断，apply 在同一 operation guard 内完成成功后的 restart，并在 facade 处把全部内部 warning prose 收敛为可组合 warningCodes。
