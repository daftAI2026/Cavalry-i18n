/**
 * [INPUT]: 依赖 install 布局、verified vendor Info.plist、mac_runtime/windows_runtime/windows_qpa、privilege typed process guard/graceful close 与 state。
 * [OUTPUT]: 提供 prepare_apply（macOS 非 English runtime 仅从完整可信 vendor Info + baseline 生成；Official Restore 由 baseline-derived plan 恢复）、typed preflight、签名/app seal、English 早退与 restart 编排入口；macOS Switch/Restore preflight 只读探测运行态，不替用户关闭 Cavalry。
