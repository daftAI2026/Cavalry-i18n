...
platform_runtime.rs: 私有平台运行时编排 facade；macOS 首装切换 `CFBundleExecutable` 后把 vendor 主二进制与 ExtensionLayer/injector 一并纳入有界 nested-code 重签，再做外层 seal；Windows Program Files 已由 commands 提前分流，剩余自定义可写根在 payload 前拒绝 drift、以 typed 结果精确关闭 Cavalry 并验证直接写权限，在 pending JSON/generic 后执行 QPA ACTIVE 或显式 English 恢复，最后才允许 final marker；restart 只交付诊断子进程环境。
