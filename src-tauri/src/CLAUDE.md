...
commands.rs: renderer API facade；保留九条稳定 Tauri command、camelCase DTO 和兼容测试 seam；恢复基线只由 `apply_language` 在同一 operation lock 的写入前自动建立；`apply_language` 以强类型 Channel 报告四个真实阶段，并只消费 macOS durable transaction 保留的 typed PermissionDenied 作为 App Management oracle，不再从普通错误文本猜测；受保护 apply commit 成功后先调用 `complete_permission_handoff_after_commit` 触发一次 reverse，再进入 `restartCavalry` 与最终业务结果，最终 `finalize_permission_handoff` 只为非权限失败 cleanup，避免 success 重复 reverse；`open_privacy_security` 只消费固定 permission、有限 CSS rect/viewport 与本 session Channel，固定设置 URL 仍由 privilege owner 打开；项目链接、About 与 updater 各自保持既有 owner。
...
macos_permission_handoff.rs: macOS App Management handoff 的 Rust session owner；把主 WebView、CSS source rect/viewport 与 session Channel 交给固定 Objective-C AppKit owner，只将 retry/dismiss/error 回送 renderer，并由受保护 apply commit 触发一次 reverse 或由非权限失败触发 cleanup，最终 success 不再由 finalizer 重复触发。
