...
commands.rs: renderer API facade；保留九条稳定 Tauri command、camelCase DTO 和兼容测试 seam；恢复基线只由 `apply_language` 在同一 operation lock 的写入前自动建立；macOS 首次 mutation admission 在任何 Cavalry 写入和业务阶段前返回 typed PermissionDenied，先进入 App Management handoff，handoff 已展示标记只防止重复首导且不冒充授权成功，随后完整 durable transaction 仍是唯一权限 oracle；受保护 apply commit 成功后先触发一次 reverse，再进入 Cavalry 启动与最终业务结果，任何失败 cleanup；`open_privacy_security` 在系统设置成功打开后耐久记录“已展示”；status/apply/权限设置边界向本地有界 diagnostics JSONL 投影脱敏结果。
...
diagnostics.rs: 本地支持诊断事实层；在 state 目录以两代 512 KiB JSONL 记录启动恢复、状态裁决、语言事务与 App Management 设置边界，脱敏用户/临时/state 路径且不记录语言内容、密钥、哈希或 TCC 数据，写入失败永不改变业务结果。
...
mac_official.rs: macOS vendor baseline 真相层；从严格 identity、vendor codesign、English JSON 与 Info/main/CodeResources/Extension preimages 建立同一 immutable generation，managed apply 只能从该 baseline 推导 wrapper/Keychain postimage；官方还原逐项复核 bytes/mode/absence/signature，并删除脚本入口重签产生的三个外置签名组件，避免恢复 vendor Info.plist 后留下 unsealed bundle residue。
...
macos_permission_handoff.rs: macOS App Management handoff 的 Rust session owner；除把主 WebView、CSS source rect/viewport 与 session Channel 交给固定 Objective-C AppKit owner、回送 retry/dismiss/error 外，还以 state 目录中的原子 0600 标记拥有首次 protected-write admission；该标记只证明首导已展示，历史成功 state 可跳过首导，真实权限仍由后续 durable transaction 判定；受保护 apply commit 触发 reverse，任何失败 cleanup。
platform_runtime.rs: 私有平台运行时编排 facade；macOS 首装切换 `CFBundleExecutable` 后把 vendor 主二进制与 ExtensionLayer/injector 一并纳入有界 nested-code 重签，再做外层 seal；严格证明的 Managed Legacy 不重建缺少 vendor preimage 的 runtime，只发布目标语言 final marker 并复用既有发布 postimage；Windows Program Files 已由 commands 提前分流，剩余自定义可写根执行 QPA ACTIVE 或显式 English 恢复，最后才允许 final marker；restart 只交付诊断子进程环境。
...
privilege.rs: 唯一系统命令 facade；保持既有 public API，公开 typed graceful-close、固定项目外链、Windows Program Files apply outcome，以及 macOS 脚本入口重签三个外置组件的精确路径/残留分类；startup recovery 先以不跟随 reparse 的保存根只读探针确认 journal，再持锁恢复。
