...
commands.rs: renderer API facade；保留九条稳定 Tauri command、camelCase DTO 和兼容测试 seam；恢复基线只由 `apply_language` 在同一 operation lock 的写入前自动建立；`apply_language` 以强类型 Channel 报告四个真实阶段，并作为 App Management handoff 的唯一成功 oracle；`open_privacy_security` 只消费固定 permission、有限 CSS rect/viewport 与本 session Channel，固定设置 URL 仍由 privilege owner 打开；项目链接、About 与 updater 各自保持既有 owner。
