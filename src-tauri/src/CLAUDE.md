...
commands.rs: renderer API facade；保留九条稳定 Tauri command、camelCase DTO 和兼容测试 seam；恢复基线只由 `apply_language` 在同一 operation lock 的写入前自动建立；`apply_language` 以强类型 Channel 报告四个真实阶段，并只消费 macOS durable transaction 保留的 typed PermissionDenied 作为 App Management oracle，不再从普通错误文本猜测；`open_privacy_security` 只消费固定 permission、有限 CSS rect/viewport 与本 session Channel，固定设置 URL 仍由 privilege owner 打开；项目链接、About 与 updater 各自保持既有 owner。
...
detect.rs: 安装探测编排，保存路径始终优先；macOS 默认候选遵循用户 `~/Applications`→系统 `/Applications` 域次序，使无 root fallback 的当前用户先选择自己拥有的 bundle；Windows 再按运行进程→MSI→常见目录选择有效安装根。严格 macOS identity 通过 typed XML/binary plist 锁定 canonical root、com.scenegroup.cavalry、2.7.2 short/build、Mach-O 架构、Cavalry 与 libExtensionLayer SHA-256，official immutable revision fingerprint 排除受控 ExtensionLayer patch，并只在 signature range 精确结束于 slice/`__LINKEDIT` 末端时归一 codesign 改写的 command 与 segment extent，重复 apply 仍把其他 code/segment 变化视为 drift；签名 Team ID/designated requirement 以 typed unavailable 留给 privilege runner；Windows revision read/write 均流式计算真实 SHA，拒绝以可碰撞的 NTFS metadata 代替内容身份。
...
macos_permission_handoff.rs: macOS App Management handoff 的 Rust session owner；把主 WebView、CSS source rect/viewport 与 session Channel 交给固定 Objective-C AppKit owner，只将 retry/dismiss/error 回送 renderer，并由真实 apply 结果触发 reverse 或 cleanup。
