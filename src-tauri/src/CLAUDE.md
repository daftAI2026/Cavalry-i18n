...
成员清单
main.rs: 二进制入口；Windows 先消费 `--launch-cavalry` 原生快速路径并返回明确退出码，其余调用进入 `cavalry_i18n_tauri::run()`；后续 Program Files worker 只能在接入时同步更新本项。
lib.rs: Tauri Builder 装配层，注入 bridge 初始化脚本、注册 6 个 command、公开跨平台纯模块与 Windows headless launch/QPA 部署，并私有挂接 `platform_runtime` 以保持命令 facade 不泄漏平台 cfg 编排。
...
platform_runtime.rs: 私有平台运行时编排 facade；Windows 在 payload 前拒绝 drift/Program Files、精确关闭 Cavalry 并验证直接写权限，在 pending JSON/generic 后执行 QPA ACTIVE 或显式 English 恢复，最后才允许 final marker；restart 只交付诊断子进程环境。
