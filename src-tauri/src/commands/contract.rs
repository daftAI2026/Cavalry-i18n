 ...
 * [OUTPUT]: 提供九命令名称、renderer 兼容 payload DTO、固定 App Management handoff 请求/结果、四阶段有序进度事件、启动恢复显式阻断诊断、Status 的版本兼容/官方恢复能力/首次 macOS handoff admission 与 Windows residue 标记，以及可组合的稳定 errorCode/warningCodes 投影；内部签名清理事实不进入 renderer 契约。
