/**
 * [INPUT]: 依赖 packaged English source、legacy/immutable JSON snapshot gate、install identity 与 Windows QPA 只读证据；Stock 旧状态通过只读 restore plan 同时证明 vendor qwindows 和 generic 所有权。
 * [OUTPUT]: 提供 Windows legacy snapshot 的只读可信识别及 apply 阶段的 immutable English generation 迁移；若 generation 已发布而语言事务尚未提交 provenance，则严格复证后直接关联同一 generation。macOS 不从旧 JSON-only 快照推断 runtime 所有权。
