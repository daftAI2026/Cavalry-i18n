...
contract.rs: renderer 兼容 DTO 与六命令常量；集中 camelCase JSON 序列化、稳定 errorCode/可组合 warningCodes、`reconciliationRequired` 与 UAC `permissionRequired` 投影及内部 warning prose→code 收敛，facade 返回前清空原文，禁止把底层临时路径泄漏到 UI。
...
snapshot.rs: English 安装真相与快照/provenance 闸门；macOS 只从 clean vendor identity/signature 建 unified baseline generation，状态轮询只读；state durability warning 锁定 renderer mutation，显式 Refresh 即使 snapshot state no-op 也重新 fsync state 目录，成功后才解除；Windows 以 38 份 JSON + 精确原厂 QPA 识别厂商重装造成的 stale marker/runtime，Refresh 只采集并返回 `reconciliationRequired`，不执行安装收敛。
