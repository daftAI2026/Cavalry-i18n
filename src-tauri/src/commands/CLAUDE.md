...
status.rs: 安装发现、只读状态投影、显式 control-state recovery 与权限动作判定；非支持版本永不进入 mutation；macOS Official 标签仍要求 vendor signature，但可恢复的自签 stock runtime 投影 `recoverableStock`，Managed Legacy 的准入由已发布 postimage 与可信 English generation 决定，两者都不再把 Team ID 当翻译许可证；首次受支持安装以 state-dir 标记投影 `macosPermissionHandoffRequired`，stock runtime 中出现任一已知外置签名组件即投影 `macosSignatureResidueRepairable`，目录内无关成员不取得翻译控制权；Windows status 继续从安装现实重算 residue。
