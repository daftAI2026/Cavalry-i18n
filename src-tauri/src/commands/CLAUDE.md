...
成员清单
apply.rs: 语言写入事务编排；保持 `apply_language_inner` 公开测试 seam；在真实验证、English 基线、跨平台事务提交边界使用 RAII phase guard；Windows 生产/测试共用 pair 构造；macOS 官方路径继续使用完整 vendor baseline，Managed Legacy 只复用可信 generation/postimage；只有 clean vendor runtime、strict codesign 因 unsealed 失败且 `_CodeSignature` 精确为 `CodeResources + CodeDirectory + CodeSignature + CodeRequirements` 时，才先用同一 durable transaction 删除旧 Switcher 残留并重新验证 vendor identity，任何未知集合仍 fail closed；App Management 只由事务层 typed PermissionDenied 进入 renderer。
...
contract.rs: renderer 兼容 DTO、九命令常量与操作事件合同；集中 camelCase JSON、稳定 error/warning codes，以及 Status 的 `managedLegacy`、`officialRecoveryAvailable`、`macosPermissionHandoffRequired`、`macosSignatureResidueRepairable`、四态版本兼容与 Windows residue/UAC 投影；两个 macOS 布尔值都只承载后端证明，不允许 renderer 自行推断；Channel 失败不改变已提交事务，facade 返回前清空原文。
...
status.rs: 安装发现、只读状态投影、显式 control-state recovery 与权限动作判定；非支持版本永不进入 mutation；macOS official 要求 vendor signature，Managed Legacy 要求已发布 postimage 与可信 English generation；首次受支持安装以 state-dir 标记投影 `macosPermissionHandoffRequired`，精确旧版外置签名残留只在 clean vendor runtime + exact file set 时投影 `macosSignatureResidueRepairable`，其他 strict signature failure 仍为 `modifiedOrUnverified`；每次状态读取记录无路径/哈希裁决，Windows status 继续从安装现实重算 residue。
