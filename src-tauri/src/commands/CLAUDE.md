...
成员清单
apply.rs: 语言写入事务编排；English 快速返回仅接受精确 Clean disposition，stale marker 必须继续 pending→runtime→final 事务；以长度、只读位、Unix mode 与内容筛除未变化 pair，Windows 从已证明的 English snapshot 为四语言统一生成 canonical pretty overlay，使 Program Files source provenance 可精确重建，并把任意安装根写入前 Cavalry 仍运行统一投影为稳定 errorCode；自定义可写根仍按同一顺序直写，macOS English 继续复制原始 snapshot。
