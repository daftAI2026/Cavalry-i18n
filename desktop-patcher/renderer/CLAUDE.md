...
成员清单
index.html: 静态 DOM 骨架，固定 `appVersion`、`appPath`、`currentLanguage`、`languageSelect`、`browseButton`、`extractButton`、`applyButton`、`permissionButton`、`statusText` 等迁移锚点。
styles.css: 唯一样式源，定义窗口布局、按钮、权限按钮、状态面板、自定义 select 与视觉 token。
app.js: 唯一交互源，调用 `window.cavalryI18n` 完成状态读取、选择 app、刷新 English、应用语言、权限等待态与重启。
