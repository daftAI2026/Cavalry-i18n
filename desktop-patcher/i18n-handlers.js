/**
 * [INPUT]: 依赖 node fs/os/path 与可注入 command runner，依赖 ./lib/* 完成应用探测、补丁复制与提权写入
 * [OUTPUT]: 对外提供 createI18nHandlers 与 registerI18nHandlers，用 5 个 IPC handler 承载 renderer 契约
 * [POS]: desktop-patcher 的业务装配层，被 Electron main 与无副作用测试 harness 共同消费
...
let spawnSync = defaultSpawnSync;
let dialog = null;
...
let languagesDir = path.join(repoRoot, 'languages');
let resourcesPath = process.resourcesPath || '';
let isPackaged = false;
let platform = process.platform;
let now = () => new Date();
let getUserData = () => {
  throw new Error('appPaths.getUserData dependency is required.');
};
let findCavalryApp = detect.findCavalryApp;
let getDefaultAppCandidates = detect.getDefaultAppCandidates;
let inspectBundle = detect.inspectBundle;
let listLanguageOptions = detect.listLanguageOptions;
let readBundleVersion = detect.readBundleVersion;
let buildCopyPairs = patch.buildCopyPairs;
let extractEnglish = patch.extractEnglish;
let stageFiles = patch.stageFiles;
let verifyCodeSignature = patch.verifyCodeSignature;
let copyWithSudo = sudo.copyWithSudo;
...
function useI18nDeps(deps = {}) {
  const commandRunner = deps.commandRunner || {};
  fs = deps.fs || fs;
  os = deps.os || os;
  spawn = deps.spawn || commandRunner.spawn || spawn;
  spawnSync = deps.spawnSync || commandRunner.spawnSync || spawnSync;
  dialog = deps.dialog || dialog;
  resourcesPath = deps.resourcesPath || resourcesPath;
  platform = deps.platform || platform;
  now = deps.now || now;
  languagesDir = deps.languagesDir || path.join(deps.repoRoot || repoRoot, 'languages');

  const appPaths = deps.appPaths || {};
  if (typeof appPaths.getUserData === 'function') {
    getUserData = appPaths.getUserData;
  }
  if (typeof appPaths.isPackaged === 'function') {
    isPackaged = Boolean(appPaths.isPackaged());
  } else if (typeof appPaths.isPackaged === 'boolean') {
    isPackaged = appPaths.isPackaged;
  }

  findCavalryApp = deps.findCavalryApp || findCavalryApp;
  getDefaultAppCandidates = deps.getDefaultAppCandidates || getDefaultAppCandidates;
  inspectBundle = deps.inspectBundle || inspectBundle;
  listLanguageOptions = deps.listLanguageOptions || listLanguageOptions;
  readBundleVersion = deps.readBundleVersion || readBundleVersion;
  buildCopyPairs = deps.buildCopyPairs || buildCopyPairs;
  extractEnglish = deps.extractEnglish || extractEnglish;
  stageFiles = deps.stageFiles || stageFiles;
  verifyCodeSignature = deps.verifyCodeSignature || verifyCodeSignature;
  copyWithSudo = deps.copyWithSudo || copyWithSudo;
}
...
function getPackagedInjectorPath() {
  return path.join(resourcesPath, 'injector', INJECTOR_DYLIB_NAME);
}
...
async function browseAppHandler() {
...
async function extractEnglishHandler(_event, payload) {
  try {
...
async function applyLanguageHandler(_event, payload) {
  try {
    ...
    const stagingRoot = path.join(os.tmpdir(), `cavalry-i18n-staging-${now().getTime()}-${process.pid}`);
    let copyMode = 'shell';
    try {
      const runtimeSourceDir = path.join(stagingRoot, 'runtime');
      const stagedFilesDir = path.join(stagingRoot, 'staged');
      const runtimePairs =
        platform === 'darwin' ? buildMacRuntimePairs(appPath, lang, runtimeSourceDir) : [];
...
async function restartCavalryHandler(_event, payload) {
  try {
    const storedState = readState() || normalizeState();
    const appPath = payload?.appPath || findCavalryApp(storedState.appPath);
    if (!appPath) {
...
function createI18nHandlers(deps = {}) {
  useI18nDeps(deps);
  return {
    'i18n:get-status': getStatusHandler,
    'i18n:browse-app': browseAppHandler,
    'i18n:extract-english': extractEnglishHandler,
    'i18n:apply-language': applyLanguageHandler,
    'i18n:restart-cavalry': restartCavalryHandler,
  };
}

function registerI18nHandlers(ipcMain, deps = {}) {
  const handlers = createI18nHandlers(deps);
...
module.exports = {
  ...
  buildLaunchWrapper,
  buildMacRuntimePairs,
  normalizeState,
  restartCavalryBundle,
};
