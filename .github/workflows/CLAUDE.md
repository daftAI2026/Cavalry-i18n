...
成员清单
build.yml: 主 CI/CD 工作流，支持手动触发、main/PR/tag 自动触发；先校验版本元数据，再跑 Node/Rust 合同测试，并在 macOS 上构建 Tauri DMG 与 `.app` artifact，`v*` tag 以产品名标题和手写下载说明发布 DMG 到 GitHub Releases。
...
2026-05-15: release job 改为 `Cavalry Language Switcher vX.Y.Z` 标题和手写 release notes，避免只输出自动 changelog 链接。
