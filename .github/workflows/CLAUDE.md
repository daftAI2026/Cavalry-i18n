...
成员清单
build.yml: 主 CI/CD 工作流，支持手动触发、main/PR/tag 自动触发；先校验版本元数据，再跑 Node/Rust 合同测试，并在 macOS 上构建 Tauri DMG、图标保真 `.dmg.zip` 与 `.app` artifact，`v*` tag 发布 DMG/DMG zip 到 GitHub Releases。
...
2026-05-15: packaging artifact 与 tag Release 增加 `*.dmg.zip`，保留 Finder 图标资源分叉，避免 GitHub 裸 DMG 下载只剩 data fork。
