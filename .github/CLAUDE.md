...
成员清单
workflows/: CI/CD 工作流目录，负责无原生库源码闭包、跨平台 Qt/runtime 现场构建、Rust/合同/漏洞/acceptance 门与 Windows NSIS 生命周期；tag 生成双架构 ad-hoc macOS 包，并以独立 Tauri updater 私钥生成 macOS archive/签名和 Windows NSIS 签名，汇合 `latest.json` 后通过 schema v6 九资产 seal/private-draft exact readback 发布；普通 PR/main 不接触 updater 私钥，显式 no-tag signing smoke 仅在受保护环境生成 macOS 候选并用客户端公钥验签，Developer ID/notarization 不冒充当前能力。
