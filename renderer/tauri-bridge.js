 ...
 * [OUTPUT]: 冻结最小 window.cavalryI18n API；Apply Channel 只转发 verify/baseline/apply/restart，Status 保留安装/版本兼容/官方恢复能力、后端已证明 clean vendor runtime 与精确三个旧 Switcher 外置签名组件边界的 macOS 残留标记及权限 handoff 前置标记，权限入口只接受固定 App Management 与有限 CSS rect，Updater Channel 只转发安全阶段与计数；其余接口仅转发固定 project-link、About 与 main-window caption，丢弃 raw warning、URL/签名/路径/原始响应。
...
(() => {
  ...
  function normalizeStatus(result) {
    ...
    return {
      ...
      macosSignatureResidueRepairable: result.platform === 'macos' && result.macosSignatureResidueRepairable === true,
