...
app.js: 唯一业务交互源；文件夹动作只在后端未发现安装时显露，Select 保留但禁用当前语言，非支持版本进入只读门禁；Managed Legacy 继续四语切换；macOS 精确旧版签名残留只消费后端 `macosSignatureResidueRepairable`，允许 Switch/Restore 进入真实事务清理，未知签名异常仍要求重装；后端 `macosPermissionHandoffRequired` 为真时，Switch/Restore 在调用 `applyLanguage` 前先进入既有 handoff，设置打开与 drop 均不冒充已授权，file-URL drop 只触发一次真实 oracle；后续 typed PermissionDenied 保留 Activity 历史并要求重开；Updater、Cavalry 打开阶段、Toast 继续保持各自语义边界。
...
ui-text.js: 稳定的 English/简体中文/繁体中文/日文 renderer 文案与 `STATUS_TITLE_KEYS` 状态标题路由；覆盖 Select、Badge、Switch/Restore、Updater、真实阶段、精确旧版签名残留与首次权限前导；未知安装完整性失败仍给出重开后再官方重装的恢复路径，权限文案只陈述用户动作，不声称设置已授予权限；AlertDialog 遵循“结果/风险在标题，影响/恢复在正文”。
