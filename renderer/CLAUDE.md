...
app.js: 唯一业务交互源；文件夹动作只在后端未发现安装、`appPath` 为空时显露，任何已发现安装都保持摘要只读，恢复阻塞由 Activity/AlertDialog 承载；Select 初始只显示本地化占位并在明确选择前禁用 Switch，旧/新/未知 Cavalry 版本统一进入只读门禁，Managed Legacy 继续四语切换且 Restore English 自动退化为受管英文事务；macOS 只读权限未知保持 idle，只有真实写事务拒绝才先把 provisional phase error 收敛为保留已完成历史的链尾权限阻断，完成共享可读停顿后才显露 AlertDialog，并由 permission-handoff controller 在 native 接管后关闭；retry 为每次续跑分配唯一 `attemptId`，追加四语 `resumeAfterPermission` 而不重新 start Activity，受保护 apply commit 后由 Rust 先触发 reverse 再进入 restart；只有 transport rejection 才显示桌面服务不可用，About 唤起失败进入 Toast。
...
ui-text.js: 稳定的 English/简体中文/繁体中文/日文 renderer 文案与 `STATUS_TITLE_KEYS` 状态标题路由；标题、Windows caption 可访问名称、Select 占位、条件式 Official 徽章、Switch/Restore English/Update 任务上下文、四语 `resumeAfterPermission` 与真实阶段事件全部本地化；Switch 文案只表达用户目标，把后端 restart phase 诚实投影为“打开 Cavalry”，Restore 只承诺英文结果而不伪称所有路径都恢复官方 runtime，旧/新/未知版本分别保持用户升级方向与安装只读；权限文案只服务真实拒绝，并分别描述 macOS 设置动作与 Windows UAC 动作，不保留“可能需要”或泛化操作系统请求；AlertDialog 遵循“结果/风险在标题，影响/恢复在正文”。
