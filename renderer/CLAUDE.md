...
app.js: 唯一交互源，按系统语言本土化 UI，调用冻结的 `window.cavalryI18n` 完成跨平台状态读取、安装位置选择、只读刷新 English 快照、显示 Windows typed residue warning、通过现有普通 apply `en` 事务恢复英文、English UI/翻译应用与独立官方还原确认及权限等待态；English refresh 不关闭 Cavalry、不触发 UAC、不执行 apply 或安装事务恢复；英文快照未可信存在或控件被阻断时禁用英文恢复；macOS `modifiedOrUnverified + needsExtract` 表示原始 English provenance 不完整，禁用 extract/apply/official restore、直接显示四语官方重装并重新选择安装位置指引；所有 warning 只消费可组合 `warningCodes` 并逐项本土化，禁止显示 raw warning prose。`stateDurabilityPending` 时仅保留 Refresh English，成功 no-op durability reconfirm 后才解锁其他 mutation controls。所有写入均经普通“应用并重启”事务，transport 拒绝以本地化状态恢复。
