...
icons.js: Renderer 单一语义图标注册表，集中保存经 MIT 归因的精简 Phosphor Regular SVG path 与项目自有权限 handoff 实心箭头，并只暴露冻结 `create(name)` 工厂；Restore 映射 FloppyDiskBack，打开 Cavalry 映射 Play，事件拖入映射 ArrowUp，权限视觉提示使用独立 clean-room glyph，Windows caption 映射 Minus/Square/Copy/X；事件、Updater、Toast 与窗口控件只消费语义名，不接管应用 Logo 或 macOS 交通灯。
...
app.js: 唯一业务交互源；文件夹动作只在后端未发现安装、`appPath` 为空时显露，任何已发现安装都保持摘要只读，恢复阻塞由 Activity/AlertDialog 承载；Select 初始只显示本地化占位、排除 English 并保留但禁用当前语言，并在明确选择前禁用 Switch，旧/新/未知 Cavalry 版本统一进入只读门禁；Managed Legacy 继续四语切换，非英文时 Restore English 自动退化为受管英文事务，已经处于 English 时禁用无结果差异的 Restore，只有 Windows English residue 例外保留清理动作；macOS 只读权限未知保持 idle，只有真实写事务拒绝才先把 provisional phase error 收敛为保留已完成历史的链尾权限阻断，完成共享可读停顿后才显露 AlertDialog，并由 permission-handoff controller 在 native 接管后关闭；file-URL drop 只触发一次同进程真实 oracle，重复成功的安装验证/恢复文件前置阶段不再生成第二组 Marker，真实前置失败仍投影；若 oracle 仍拒绝则隐藏 Retry 并在 Activity 明确要求重新打开语言切换器；系统“退出并重新打开”若成功重开，新会话按普通首屏开始，不持久化或自动执行旧意图；Updater 确认只说明版本、替换/重启影响与 macOS ad-hoc 后处理，不把 manifest changelog 塞入紧凑对话框；受保护 apply commit 后才由 Rust 进入与授权无关的 Cavalry 打开阶段，并以 Phosphor Play 表达启动而非循环重启；只有 transport rejection 才显示桌面服务不可用，About 唤起失败进入 Toast。
tauri-bridge.js: 非视觉兼容桥，在业务脚本前定义最小冻结 API；归一化 camelCase payload、稳定 `warningCodes`/updater error codes、四态 `versionCompatibility`、`officialRecoveryAvailable`、Action/Status、脱敏 Update DTO 与 downloading/installing/restarting 有序事件；App Management 入口只发送固定 permission、有限 source rect/CSS viewport 与独立 ordered Channel，不接受任意设置 URL；并将单一 About 唤起、固定 `main` label 的 minimize/toggle/is-maximized/close 暴露给各自状态机。
ui-text.js: 稳定的 English/简体中文/繁体中文/日文 renderer 文案与 `STATUS_TITLE_KEYS` 状态标题路由；标题、Windows caption 可访问名称、Select 占位、条件式 Official 徽章、Switch/Restore English/Update 任务上下文与真实阶段事件全部本地化；Switch 文案只表达用户目标，把后端 restart phase 诚实投影为“打开 Cavalry”，Restore 的标题、正文与按钮统一承诺英文结果，不用“恢复 Cavalry”伪称所有路径都恢复官方 runtime，旧/新/未知版本分别保持用户升级方向与安装只读；真实安装验证失败先要求重开 Switcher，仍失败才引导从官方安装器重装后端声明的受支持版本；权限文案只服务真实拒绝，分别描述 macOS 设置、App Management 退出后生效与 Windows UAC 动作，不保留“可能需要”、合成的权限续跑旁白或泛化操作系统请求；AlertDialog 遵循“结果/风险在标题，影响/恢复在正文”。
