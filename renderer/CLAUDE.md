...
operation-log.css: 三轨任务视窗的独立视觉层，只消费 tokens.css；中性外框复用 12px panel padding、border 与 radius，idle 单轨双轴居中，running 以 Grid 固定首尾 14/20 Message 并只让中段 Marker 滚动；中段按 shadcn Base Marker、Spinner、shimmer 复刻 16px 单色 marker 与统一 8px 关系间距，新行只以不改变布局的 opacity/transform 入场，8px scroll-fade 不裁切外框或首尾。
