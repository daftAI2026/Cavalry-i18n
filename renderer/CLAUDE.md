...
tokens.css: 离线设计角色唯一真相源；16/14/13px、400/450/500 与 4px 节奏覆盖窗口/控件/Activity/Toast/AlertDialog，About 标识由 64px 基准加一个 4px 档得到 68px。语言 blue-subtle 与 Official green-subtle 使用透明边界，基础灰色可保留描边；Activity 的 warning/danger 只服务语义图标，Toast 保留 shadcn 16px inset 并与 20px 主网格错层。
...
icons.js: Renderer 单一语义图标注册表，集中保存经 MIT 归因的精简 Phosphor Regular SVG path，并只暴露冻结 `create(name)` 工厂；Restore 映射 FloppyDiskBack，Windows caption 映射 Minus/Square/Copy/X；事件、Updater、Toast 与窗口控件只消费语义名，不接管应用 Logo 或 macOS 交通灯。
