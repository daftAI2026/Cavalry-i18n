...
operation-log.css: 任务事件视窗的独立视觉层，只消费 tokens.css 并按 shadcn Base Marker separator、Spinner、shimmer 与 scroll-fade 源码复刻有界列表、16px marker、8px 关系间距和滚动状态；首事件用 auto margin 让短记录贴底而不让 `flex-end` 吞掉溢出的旧事件，10px 滚动轨与说明行光学偏移均以 token 固化，不拥有窗口布局、业务状态或 AlertDialog。
