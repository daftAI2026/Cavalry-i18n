...
app.js: 唯一业务交互源；文件夹动作只在后端未发现安装、`appPath` 为空时显露，任何已发现安装都保持摘要只读，恢复阻塞由 Activity/AlertDialog 承载；Select 初始只显示本地化占位并在明确选择前禁用 Switch，旧/新/未知 Cavalry 版本统一进入只读门禁，Managed Legacy 继续四语切换且 Restore English 自动退化为受管英文事务；macOS 只读权限未知保持 idle，只有真实写事务拒绝才先把 provisional phase error 收敛为保留已完成历史的链尾权限阻断，完成共享可读停顿后才显露 AlertDialog，并由 permission-handoff controller 在 native 接管后关闭；file-URL drop 只触发一次同进程真实 oracle，若仍拒绝则隐藏 Retry 并在 Activity 明确要求重新打开语言切换器；系统“退出并重新打开”若成功重开，新会话按普通首屏开始，不持久化或自动执行旧意图；受保护 apply commit 后才由 Rust 进入与授权无关的 Cavalry 打开阶段；只有 transport rejection 才显示桌面服务不可用，About 唤起失败进入 Toast。
