...
tauri-bridge.js: 非视觉兼容桥，在 Tauri 壳里于 `app.js` 前定义 `window.cavalryI18n`，归一化 `appManagementGranted` 并补充 `openPrivacySecurity`。

依赖边界:
renderer 不知道 Tauri command 细节；它只依赖 `window.cavalryI18n` 兼容 API。Tauri bridge 必须在 `app.js` 执行前注入。
