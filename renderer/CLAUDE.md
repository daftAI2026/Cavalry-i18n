...
operation-log.css: 任务事件视窗的独立视觉层，只消费 tokens.css；中性外框复用 panel padding、border 与 radius 角色，scroll-fade 仅裁切框内滚动内容，不遮蔽容器边界；内部按 shadcn Base Marker separator、Spinner 与 shimmer 源码复刻有界列表、16px marker、8px 关系间距和滚动状态，首事件用 auto margin 让短记录贴底而不让 `flex-end` 吞掉溢出的旧事件，不拥有业务状态或 AlertDialog 语义。
operation-log.js: 无依赖任务事件 DOM 投影器，以空 separator 表达 idle、带标签 separator 建立任务上下文，再维护稳定 id 的有序 upsert/replace；运行态组合 Spinner 与 shimmer，终态恢复检查安装、恢复文件、翻译事务、重启或更新各自的 Phosphor 语义图标，新事件让视窗跟随到底部；只使用 textContent，不读取 Tauri、业务状态或自行推进阶段。
