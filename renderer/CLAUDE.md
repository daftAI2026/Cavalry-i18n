...
tauri-bridge.js: 非视觉兼容桥，在业务脚本前定义最小冻结 API；归一化 camelCase payload、稳定 warning/updater codes、四态版本、官方恢复、固定 false 的旧 `macosPermissionHandoffRequired` 兼容字段、Action/Status 与脱敏 Update DTO；内部签名清理事实不进入 renderer 契约，App Management 入口只发送固定 permission、有限 source rect/CSS viewport 与独立 Channel，不接受任意设置 URL。
