...
成员清单
index.html: 跨平台语义 DOM 骨架，在 macOS Overlay 原生交通灯下只提供 40px 可拖拽标题内容，不伪造窗口按钮；安装 Item 将当前语言与 macOS 安装信任分成相邻双徽章，语言主任务使用隐藏原生数据槽 + combobox/listbox 语义骨架，内容按安装 Item + 三动作 Maintenance Button Group + 持久 Alert 展开，保留更新 tooltip、两种英文恢复语义、原生 dialog 和全部稳定控件锚点；只加载本地资源。
styles.css: 唯一样式源，以平台系统字体、36px 控件、7/9px 圆角、20px 窗口四周 padding、16px 板块节奏与语义色实现 460×428 默认桌面布局；二维结构和动作回流由 Grid 拥有，标题栏、徽章与按钮内部关系由 Flex 拥有，16px 原生交通灯在 40px 标题栏中上下各留 12px，内容左缘与其 20px 中心线对齐；持久 Alert 按正式生产文案自然增高，不为伪造诊断定高或建立嵌套滚动，极端溢出只由主内容区兜底；选择器键盘焦点只改变细边框而不绘制外圈，不含交通灯绘制、CDN、字体包或组件运行时。
