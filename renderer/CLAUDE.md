...
operation-log.js: 依赖 icons.js 的任务事件 DOM 投影器，以空 separator 表达 idle、带标签 separator 建立任务上下文，再维护稳定 id 的有序 upsert/replace；不足一屏时保持 scrollTop=0，触底后才跟随最新事件，运行态组合 SpinnerGap 与 shimmer，终态按语义名请求验证、归档、翻译、恢复、下载、安装或重启图标；只使用 textContent，不持有 SVG path、不读取 Tauri、业务状态或自行推进阶段。
