...
styles.css: 跨平台共享 shell、安装 Item、Select、Tooltip、按钮和 AlertDialog 视觉实现，只消费 tokens.css 且不定义私有设计常量；16/14/13px 与 400/450/500 复用系统字体排印角色。Grid 管 400×480 窗口内的 shell、安装卡片、Select/双动作复合轨道与 Dialog 结构，Flex 管标题/徽章/路径/按钮等一维关系；安装名称以 16/20 与同高 Badge 收紧摘要为 68px，路径以前缀弹性省略并保留末段，安装摘要与任务框统一使用 10px 大容器圆角；Tooltip 采用 shadcn Base Nova 的 Popup/Arrow 几何并由状态机提供 portal 定位；内容宽 360px、四边 20px、动作轨道为 170px + 20px + 170px，标题栏为 40px（交通灯 16px、上下各 12px），结构 gap 为 8px，并以 64px 原生占位补偿点击盒/SVG 内缩，使“绿灯可见右缘—标题字形—升级圆环可见左缘”两段实体间距同为约 12px；主窗口禁止滚动，Select 列表与任务事件视窗各自在自身边界内滚动。
