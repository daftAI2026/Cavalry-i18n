...
- English may be captured only after every `CORE_MAP` file passes packaged-English overlay equality. On Windows, a missing or known translated marker may be classified stale only when the exact vendor QPA proves an English runtime; status projection stays read-only and Refresh English must converge marker/runtime through the normal transaction. Pending, invalid, empty, ACTIVE, drifted, or unknown runtime evidence fails closed.
...
- Windows QPA activation is locked to Cavalry 2.7.2, the selected `Cavalry.exe` digest, Qt 6.6.3, x64, the verified vendor `qwindows.dll` digest, and a strict install-root manifest. Publish the durable vendor backup before atomically replacing the root DLL; `prepared`/`restoring` states delegate to the vendor QPA without translation, and `active` translates only when Cavalry/proxy/vendor/generic hashes plus the final language marker agree. Normal exit and same-version update preserve translation. Interactive Switcher uninstall explicitly offers control-plane-only removal (keep translation) or transactional English restore plus manifest-owned generic/QPA cleanup; silent/passive/update uninstall preserves translation. Unknown generic/QPA files and newer vendor DLLs are never deleted or overwritten, and cleanup failure aborts uninstall.
