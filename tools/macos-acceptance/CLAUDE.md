...
source_contract.js: producer/verifier 共用的验收源码真相；冻结完整 tracked JS/Objective-C++/Swift/build、共享翻译/输入/Quick Add context/搜索/Classic/Classic 评分及 Quick Add 描述、macOS ABI adapter 与 macOS-only 显示策略源闭包（含 English 与三语 nodeStrings/plugins JSON）、三份媒体 fixture 与 Guide 文件，避免各端维护不同 allowlist 后漏绑执行代码。
