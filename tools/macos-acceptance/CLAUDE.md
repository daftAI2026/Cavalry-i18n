...
check_contract.test.js: 跨平台静态合同；锁定 tracked source closure（含共享 Classic 评分及 macOS ABI adapter）、MH_EXECUTE 主映像选择（防止注入 dylib 占据 dyld image 0）与 GEB、800 行上限、host 缺失/篡改拒绝、真实媒体、exact-window 协议及 live 参数失败关闭，不冒充现场 PASS。
...
source_contract.js: producer/verifier 共用的验收源码真相；冻结完整 tracked JS/Objective-C++/Swift/build、共享翻译/输入/Quick Add context/顶部标签/搜索/Classic/Classic 评分及 Quick Add 描述、macOS ABI adapter 与 macOS-only 显示策略源闭包（含 English 与三语 nodeStrings/plugins JSON）、三份媒体 fixture 与 Guide 文件，避免各端维护不同 allowlist 后漏绑执行代码。
