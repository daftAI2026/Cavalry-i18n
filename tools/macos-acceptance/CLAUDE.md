...
fixtures/replace-source.png: Assets Replace/Create 的 64×48 蓝色 identity fixture；由保留的 ffmpeg 配方再生，SHA-256 与最终 session 冻结输入一致。
fixtures/dynamic-proof-two.png: Assets 动态模板的 64×48 红色第二 identity fixture；与 replace-source 内容、stem 均不同。
...
- fixture 从保留的 ffmpeg 配方确定性再生，并逐项匹配最终 session 的 frozen SHA-256；以后运行仍重新冻结当前 Git 输入，不从已清理 Cache 借用证据。
