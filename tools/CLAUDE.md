...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体、spacing token、DOM/API 锚点、macOS 原生交通灯与 Windows 无 caption/右侧控件边界、Item/Select/About/Button Group/Alert 的 Grid/Flex 职责、无业务分割线的纯留白层级、语言身份与安装状态正交双徽章、焦点、更新/About tooltip、固定项目外链、两种 English 恢复及最小窗口 capability。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/select/about/window-controls/app 执行真实 renderer 源，覆盖 Base UI Select、About 版本/固定链接、Windows caption 四命令/最大化图标/四语名称、动态 Alert、raw 诊断隔离、双徽章、updater dialog、residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 SOP/配置同构的 460×404 默认窗口、420×390 最小窗口、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability。
...
window_contract_lib.js: 窗口回归公共库，封装 AX 能力、窗口枚举、460×404 Overlay 内容截图与 `sips` 像素尺寸归一，不依赖 Python/PIL。
check_tauri_window_regression.js: packaged Tauri 主窗口回归测试，截图前后复核 AX 坐标稳定，验证 460×404 Overlay 窗口外形、完整内容截图与 backing scale，拒绝启动居中位移造成的裁切假象。
