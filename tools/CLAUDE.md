...
成员清单
classic_rank_contract_fixture.cpp: Qt 6.6.3 合成评分器与既有 Classic 搜索共同挂接，验证三语完整标题排序、英文回退、分数归还、碰撞与生命周期；不冒充原厂 ABI 或实机证据。
check_classic_rank_contract.sh: macOS 隔离编译运行共享评分 fixture，固定 Qt 6.6.3 与 QT_NO_KEYWORDS，不启动 Cavalry。
...
classify_ci_changes.test.js: CI 风险分类器离线合同；覆盖纯文档、合同/SOP、Windows、macOS、共享翻译表及翻译/输入/Quick Add 显示/搜索/Classic 搜索与评分策略的逐文件双平台门、macOS-only 原生源码、依赖/workflow、定时漏洞复审、空 diff 与未知源码/自动化路径边界。
