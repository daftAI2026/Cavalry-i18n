...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体、16/14/13px 与 400/450/500 排印角色、4px spacing token、DOM/API 锚点、macOS 原生交通灯与 Windows 无 caption/右侧控件边界、Item/Select/About/Button Group/任务视窗/AlertDialog 的 Grid/Flex 职责、权限按钮可见时才创建的条件轨道、单一语义图标注册表及 Restore→FloppyDiskBack 精确路径、idle 居中、首尾 Message/中段 Marker、Spinner/shimmer/8px scroll-fade/live-edge、真实事件可读节奏/错误抢占与阶段图标边界、当前语言 informational blue-subtle 与条件式 Official green-subtle 徽章、无确认直达 Switch、更新/About tooltip、固定项目外链与 400×484 capability。
