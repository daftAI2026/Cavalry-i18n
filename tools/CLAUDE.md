...
成员清单
check_app_contracts.js: Tauri-only Node 合同测试，承接跨平台 Python/换行、full-ui、精确版本 CHANGELOG 发布摘要、capture-only/dirty-only injector、first-match 哈希、TS message context 归属/精确空白/三语 key 对称、裸 `{}` 占位符、Qt ABI-safe accessibility 源码边界、macOS ExtensionLayer 四处空状态定点居中翻译/其余自绘英文边界、Windows MessageBar 双 caller、EditShapeTool 与 TransformTool 各三条长操作前缀及 `Space`/`Shift` 纯键位保护、Pencil/Pen/Centre text-path、selected-count QLabel 与 Pencil HTML 尾部精确替换、Time Editor 英文保护、item-model 异步补译、aboutToShow 菜单首帧、QLabel/QLineEdit fingerprint、ModalDialog、运行时图层名、自动编号 Composition 标签分母、品牌/术语、翻译质量及 package/workflow 等非壳层断言。
