...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 资源、DOM/API 锚点、macOS Overlay 原生交通灯边界、Item/自绘 Select/Button Group/Alert 布局、无外圈选择焦点、更新 tooltip、四语 Maintenance 文案与两种 English 恢复语义。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/select-control/app 执行真实 renderer 源，覆盖 Base UI 语义 open/active/selected 键盘状态、四语标题/状态/Badge、fail-closed 控件、updater tooltip/dialog、Windows residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
