...
check_renderer_contract.js: Renderer 静态合同，冻结 token 单向依赖、平台 chrome、Grid/Flex、Activity、Trigger/popup 双占位且开启后不漂移的 Select、Tooltip/AlertDialog/Toast、无描边彩色 Badge、Phosphor 图标、固定外链与 400×484 capability；权限审查页另冻结视觉参数、HTML drag/fixture 边界、retry 的唯一 `attemptId`/四语 `resumeAfterPermission`/Activity 历史保留，以及 fixture 在 `applyTransaction` commit 后、`restartCavalry` 前的 settled 控制边界，并阻止 DOM 单屏替身冒充原生 drop、授权、多屏或 backing-scale 证据。
...
ui_review_permission_handoff_runtime.js: 权限 handoff 原型的浏览器行为层；将 typed 拒绝→打开/定位设置→单次 50pt 弧线与双图交叉→实时 helper 接管/项目自绘箭头提示→整条 App row 的 DOM snapshot drag image 向整个 System Settings 区域发起 file URL copy drop→系统行视觉更新→fixture 经真实 renderer 自动重试→成功或其他错误驱动 reverse/cleanup 分开建模，并允许审查者一步触发完整结果回环而不必理解内部重试门禁；成功回环中的 `protectedApplyCommitted` 只作 review trace，不能进入生产 Activity；显式重置会重载同一生产 renderer fixture，并在新 document 重新出现真实权限动作前关闭交接入口，避免旧成功态或导航竞态污染下一轮审查；正向开始后则冻结 source，source 缺失与系统 Reduce Motion 走静态 helper，renderer 变化与舞台重排只刷新 target；仍拒绝时保留 helper，所有 DOM 视觉、单屏 CSS 几何、HTML 拖放与 fixture 结果仅是 UI Review 证据，不冒充 NSImage/NSPanel/NSDraggingSession、多屏 backing-scale 或 native 授权。
