...
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 SOP/配置同构的 460×440 默认窗口、420×402 最小窗口、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability。
...
window_contract_lib.js: 窗口回归公共库，封装 AX 能力、窗口枚举、460×440 Overlay 内容截图与 `sips` 像素尺寸归一，不依赖 Python/PIL。
check_tauri_window_regression.js: packaged Tauri 主窗口回归测试，验证 460×440 Overlay 窗口的 AX 外形与内容截图尺寸。
