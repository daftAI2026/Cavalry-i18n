...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体、16/14/13px 与 400/450/500 排印角色、4px spacing token、DOM/API 锚点、macOS 原生交通灯与 Windows 无 caption/右侧控件边界、Item/Select/About/Button Group/任务视窗/AlertDialog 的 Grid/Flex 职责、权限按钮可见时才创建的条件轨道、单一语义图标注册表、idle 居中、首尾 Message/中段 Marker、Spinner/shimmer/8px scroll-fade/live-edge、真实事件可读节奏/错误抢占与阶段图标边界、当前语言与条件式 Official 徽章、无确认直达 Switch、更新/About tooltip、固定项目外链与 400×480 capability。
check_operation_log_runtime.js: 任务反馈专属运行时合同，以最小 DOM/时钟执行真实 icons/operation-log 源，隔离验证 Marker 稳定原位更新、live-edge、快事件可读串行、结果排队和错误立即抢占，不重复 bridge 业务 fixture。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/icons/select/tooltip/path/operation-log/update-progress/about/window-controls/app 执行真实 renderer 源，覆盖冻结语义图标工厂、Base UI Select、About 版本/固定链接、Windows caption 四命令/最大化图标/四语名称、四语 idle/首尾 Message/中段 Marker、Switch 无确认直达 Apply 四阶段及 Updater 三阶段强类型有序 Channel、raw 诊断隔离、当前语言与条件式 Official 徽章、更新 Tooltip 关闭后由 AlertDialog 接管焦点、residue/UAC、macOS 权限、两种 English 恢复和 durability retry；任务反馈内部时序由专属合同负责。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 SOP/配置同构的 400×480 固定最小窗口、主窗口无滚动而 Select/任务事件视窗可内部滚动、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability。
...
window_contract_lib.js: 窗口回归公共库，封装 AX 能力、窗口枚举、400×480 Overlay 内容截图与 `sips` 像素尺寸归一，不依赖 Python/PIL。
check_tauri_window_regression.js: packaged Tauri 主窗口回归测试，截图前后复核 AX 坐标稳定，验证 400×480 Overlay 窗口外形、完整内容截图与 backing scale，拒绝启动居中位移造成的裁切假象。
