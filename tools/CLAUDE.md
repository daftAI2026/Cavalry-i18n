...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体、16/14/13px 与 400/450/500 排印角色、4px spacing token、DOM/API 锚点、macOS 原生交通灯与 Windows 无 caption/右侧控件边界、Item/Select/About/Button Group/任务事件视窗/AlertDialog 的 Grid/Flex 职责、单一语义图标注册表、separator/MarkerIcon/Spinner/shimmer/scroll-fade 与阶段图标边界、短记录顶部起排且溢出后保持最新项可达、语言身份与安装状态正交双徽章、更新/About tooltip、固定项目外链与 400×480 capability。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/icons/select/tooltip/path/operation-log/update-progress/about/window-controls/app 执行真实 renderer 源，覆盖冻结语义图标工厂、Base UI Select、About 版本/固定链接、Windows caption 四命令/最大化图标/四语名称、任务 separator 与 Spinner→事件图标原位替换、Apply 四阶段及 Updater 三阶段强类型有序 Channel、raw 诊断隔离、双徽章、updater dialog、residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
