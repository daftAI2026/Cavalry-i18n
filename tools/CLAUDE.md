...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体、16/14/13px 与 400/450/500 排印角色、4px spacing token、DOM/API 锚点、macOS 原生交通灯与 Windows 无 caption/右侧控件边界、Item/Select/About/Button Group/Activity/AlertDialog 的 Grid/Flex 职责、Activity 的 MarkerIcon/Spinner/shimmer 与阶段图标边界、无业务分割线的纯留白层级、语言身份与安装状态正交双徽章、焦点、更新/About tooltip、固定项目外链与 400×480 capability。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/select/about/window-controls/app 执行真实 renderer 源，覆盖 Base UI Select、About 版本/固定链接、Windows caption 四命令/最大化图标/四语名称、Activity 的 Spinner→阶段图标原位替换、verify/baseline/apply/restart 强类型有序 Channel、raw 诊断隔离、双徽章、updater dialog、residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 SOP/配置同构的 400×480 固定最小窗口、主窗口无滚动而 Select/Activity 可内部滚动、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability。
