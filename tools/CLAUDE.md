...
create_release_acceptance_evidence.js / create_release_acceptance_attestation.js / verify_release_acceptance_evidence.js / verify_release_acceptance_attestation.js: tag 前置 live macOS acceptance evidence 与独立 Ed25519 protected-attestation；evidence 可选携带由 Windows producer 派生的 acceptance summary，但一旦 release 声明 Windows artifact，`--require-windows` 会强制 summary 的 tag/source/installer/generic/QPA/session 绑定；候选代码只在 repo 外 prepare canonical payload、接收 detached signature/public DER 后 assemble，长期私钥只存在于不运行候选代码的外部 signer；tag 只接受 evidence+attestation 两文件提交并以外部固定 fingerprint fail-closed 校验（commit/tag/Cavalry/Qt/OS/语言矩阵）。
