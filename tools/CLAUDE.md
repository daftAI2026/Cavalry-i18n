...
ui_review_permission_handoff.js: clean-room macOS App Management 权限 handoff 的结构/样式层；以内嵌真实 `permissionMac` renderer iframe 为源、匿名 DOM mock 为系统设置目标，复用生产 tokens/Button/语义图标/应用标识，以 604px 舞台完整容纳 400×484 source、484px 设置目标与 82px helper，并在底部并列不入库的本机截图、仅供核对提示箭头的私有 Raster 与项目自绘候选；落稳后的 App 权限项与有状态已有行开关只审查 HTML 视觉连续性，控制区分别显露同进程 oracle、Later 与 Quit & Reopen fresh-session，不复制生产 Dialog、第三方私有资产或冒充原生 AppKit 实现。
ui_review_permission_handoff_runtime.js: 权限 handoff 原型的浏览器行为层；将 typed 拒绝→打开/定位设置→单次 50pt 弧线与双图交叉→实时 helper 接管/项目自绘箭头提示→整条 App row 的 DOM snapshot drag image 向整个 System Settings 区域发起 file URL copy drop→系统行视觉更新→fixture 经真实 renderer 运行一次同进程 oracle→成功 reverse、仍拒绝重开提示或其他错误 cleanup 分开建模；Later 保留生产 Activity 中的“重新打开语言切换器”阻断，Quit & Reopen 则只投影为不续跑旧任务的普通新会话，不能冒充系统已成功重开；显式重置会重载同一生产 renderer fixture，并在新 document 重新出现真实权限动作前关闭交接入口，避免旧状态或导航竞态污染下一轮审查；正向开始后冻结 source，source 缺失与系统 Reduce Motion 走静态 helper，renderer 变化与舞台重排只刷新 target；所有 DOM 视觉、单屏 CSS 几何、HTML 拖放与 fixture 结果仅是 UI Review 证据，不冒充 NSImage/NSPanel/NSDraggingSession、多屏 backing-scale、真实系统退出重开或 native 授权。
