...
resolve_windows_cmake.js: 从 `ci_action_pins.json` 的官方 Kitware/CMake v4.2.0 Windows x64 zip pin 下载并校验 archive SHA-256，重新解包受控缓存并验证 CMake/CTest 版本与布局；stdout 只输出机器可解析路径或 identity JSON，拒绝 runner PATH、低版本、floating URL、缺摘要和 archive 漂移。
