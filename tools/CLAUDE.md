...
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除既有跨平台发布合同外，锁定 release tag 与内部 SemVer 分离、人工安装/updater 资产命名、tag 已进入 origin/main 的共同 preflight、PR/main 无 vendor app 的 macOS 编译门、Windows NSIS 生命周期，以及 live acceptance 的受限证据边界。
