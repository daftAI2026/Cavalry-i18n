...
windows_nsis_provenance.js: Windows x64 NSIS 当前输入自证器；通过共享 contract 生成 schema v2 sidecar，intent 区分普通构建与 tag updater 构建，前者拒绝任意 `.exe.sig`，后者要求并绑定 exact base64 signature；只清本版本受控 EXE/provenance/signature，拒绝外国输出，并对 renderer/languages/Tauri/updater overlay/Rust/native/shared policy/generated table/generic/QPA 复算 fingerprint。
