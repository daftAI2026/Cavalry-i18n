...
check_tauri_bridge_runtime.js: Tauri bridge fake-DOM 运行合同，执行真实组件和 app 源；覆盖手动安装入口仅在未发现安装时显露、Select 占位与显式选择、主任务/Updater/About/窗口控件，并证明 About/外链失败进入局部 Toast 而不覆盖 Activity。
