...
check_renderer_contract.js: Renderer contract 测试，冻结本地 renderer 脚本顺序与 packaged hash 面、DOM id 锚点与 `window.cavalryI18n` API 需求面，覆盖英文恢复按钮、状态标签、三语言下拉与 typed residue warning，避免 Git 换行策略制造假漂移。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/app 顺序执行 renderer，覆盖 camelCase-only payload、稳定 errorCode 四语本土化、平台 dataset、英文运行态禁用恢复、非英文态 Apply/English restore 确认、提交后 cleanup warning、macOS openPrivacy/Windows requestElevation 权限恢复、不可写自定义根的无 UAC 错误、Refresh 零 apply/零 runtime mutation、bootstrap residue warning 不锁定普通 apply 与自定义 select 原生 change 语义。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除既有跨平台发布合同外，锁定 release tag 已进入 origin/main 的共同 preflight、PR/main 在无 vendor app 的 macOS Runner 编译 universal injector、Windows NSIS 默认保留数据面与交互式事务恢复英文的双语义边界，以及 Windows live full-surface 的 TEMP-owned profile/clone hash preflight、Onboarding/Adjacent 的 Qt test profile、真实 Next 转场、exact-HWND、producer-side PNG、step5-ack-only 与 exact-PID cleanup 边界。
...
create_release_acceptance_evidence.js / create_release_acceptance_attestation.js / verify_release_acceptance_evidence.js / verify_release_acceptance_attestation.js: tag 前置 live macOS acceptance evidence 与独立 Ed25519 protected-attestation；release evidence/verify 只接受 `--windows-session-dir` 原始 Windows session，并在边界重新验证后派生/比较 acceptance summary，拒绝可手写的 summary 路径；一旦 release 声明 Windows artifact，`--require-windows` 会强制 evidence 内 summary 的 tag/source/installer/generic/QPA/session 绑定；候选代码只在 repo 外 prepare canonical payload、接收 detached signature/public DER 后 assemble，长期私钥只存在于不运行候选代码的外部 signer；tag 只接受 evidence+attestation 两文件提交并以外部固定 fingerprint fail-closed 校验（commit/tag/Cavalry/Qt/OS/语言矩阵）。
