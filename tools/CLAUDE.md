...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 系统字体策略、统一 spacing token、DOM/API 锚点、macOS Overlay 原生交通灯边界、Item/自绘 Select/Button Group/动态语义 Alert 的 Grid/Flex 职责、语言/安装双徽章、无外圈选择焦点、状态标题路由、raw prose 禁入、更新 tooltip 与两种 English 恢复语义。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/select-control/app 执行真实 renderer 源，覆盖 Base UI 语义 open/active/selected 键盘状态、四语动态 Alert 标题/正文、raw backend 诊断隔离、真实语言/安装双徽章投影、fail-closed 控件、updater tooltip/dialog、Windows residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 SOP/配置同构的 460×428 默认窗口、420×390 最小窗口、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability。
...
window_contract_lib.js: 窗口回归公共库，封装 AX 能力、窗口枚举、460×428 Overlay 内容截图与 `sips` 像素尺寸归一，不依赖 Python/PIL。
check_tauri_window_regression.js: packaged Tauri 主窗口回归测试，截图前后复核 AX 坐标稳定，验证 460×428 Overlay 窗口外形、完整内容截图与 backing scale，拒绝启动居中位移造成的裁切假象。
