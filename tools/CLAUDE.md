...
window_contract_lib.js: 窗口回归公共库，封装 AX 权限探测、精确 PID 绑定的窗口枚举/聚焦/resize/关闭、原生交通灯几何与 About 菜单操作，以及 400×484 Overlay 内容截图与 `sips` 像素尺寸归一，不依赖 Python/PIL。
check_tauri_window_regression.js: packaged Tauri 主窗口 macOS 回归测试；以 launchTauri 子进程 PID 绑定首次显示、resize/restore、About 开关与截图，要求主窗口和 About 的三个原生交通灯中心距窗口顶端为 `TITLEBAR_HEIGHT / 2`（40/2=20pt，容差 1pt），再验证 400×484 Overlay 外形、完整内容区截图与 backing scale，拒绝同名已安装 App 污染证据。
