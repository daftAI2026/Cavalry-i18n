...
check_renderer_contract.js: Renderer 静态合同，冻结 token 单向依赖、平台 chrome、主窗口 10px 同行动作关系、无重复视觉标题的 About、跨平台 reconciliation Restore、Grid/Flex、Activity、Trigger/popup 双占位且开启后不漂移的 Select、Tooltip/AlertDialog/Toast、无描边彩色 Badge、Phosphor 图标、固定外链与 400×484 capability；权限审查页另冻结视觉参数、HTML drag/fixture 边界、retry 的唯一 `attemptId`、Activity 历史保留及只追加真实后端阶段，并锁定瞬时 Alert 正向来源、持久 Activity 返回锚点、显式 Back reverse 与业务 settled cleanup，阻止合成权限旁白与 DOM 单屏替身冒充原生 drop、授权、多屏或 backing-scale 证据。
