...
check_renderer_contract.js: Renderer 静态合同，冻结 token 单向依赖、平台 chrome、主窗口 10px 同行动作关系、无重复视觉标题的 About、Grid/Flex、Activity、Trigger/popup 双占位且开启后不漂移的 Select、Tooltip/AlertDialog/Toast、无描边彩色 Badge、Phosphor 图标、固定外链与 400×484 capability；权限审查页另冻结视觉参数、HTML drag/fixture 边界、retry 的唯一 `attemptId`、Activity 历史保留及只追加真实后端阶段，并锁定瞬时 Alert 正向来源、持久 Activity 返回锚点、显式 Back reverse 与业务 settled cleanup，阻止合成权限旁白与 DOM 单屏替身冒充原生 drop、授权、多屏或 backing-scale 证据。
...
check_tauri_bridge_runtime.js: Tauri bridge fake-DOM 运行合同，执行真实组件和 app 源；覆盖手动安装入口、保留但禁用当前语言的 Select Trigger/popup 占位与显式选择、Managed Legacy 受管英文恢复、旧/新/未知版本只读门禁、旧 macOS preflight hint 不再阻断真实事务、只读权限未知的 idle 投影、typed PermissionDenied 的平台恢复以及 forward/return rect 投影、主任务/Updater/About/窗口控件，锁定更新确认不内嵌 manifest changelog，并证明 About/外链失败进入局部 Toast 而不覆盖 Activity。
