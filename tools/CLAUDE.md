...
成员清单
check_app_contracts.js: Tauri-only Node 合同测试，承接 full-ui、injector、Qt ABI-safe accessibility 探测、ExtensionLayer 保留英文且不注册空补丁、懒加载 QMenu 首次绘制前翻译、动态 QLabel 浮动标题、运行时生成图层名、认证倒计时状态句、Tips 富文本标签、冒号与 No-prefix 标签兜底、Forge 动力学术语、ModelDisplay 中英间距、翻译质量、package/workflow 等非壳层断言。
