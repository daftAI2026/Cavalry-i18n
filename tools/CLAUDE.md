...
ui_review_permission_handoff_runtime.js: 权限 handoff 原型的浏览器行为层；将 typed 拒绝→打开/定位设置→单次 50pt 弧线与双图交叉→实时 helper 接管/项目自绘箭头提示→App 图标 drag image 向整个 System Settings 区域发起 file URL copy drop→系统行视觉更新→fixture 经真实 renderer 自动重试→成功或其他错误驱动 reverse/cleanup 分开建模，并允许审查者一步触发完整结果回环而不必理解内部重试门禁；显式重置会重载同一生产 renderer fixture，并在新 document 重新出现真实权限动作前关闭交接入口，避免旧成功态或导航竞态污染下一轮审查；正向开始后则冻结 source，source 缺失与系统 Reduce Motion 走静态 helper，renderer 变化与舞台重排只刷新 target；仍拒绝时保留 helper，所有 DOM 视觉、单屏 CSS 几何、HTML 拖放与 fixture 结果仅是 UI Review 证据，不冒充 NSImage/NSPanel/NSDraggingSession、多屏 backing-scale 或 native 授权。
...
macos-handoff-acceptance/: packaged App Management handoff 的证据门；read-only baseline 可省略 live contract，但所有 session 都锁定 clean detached exact source，非 read-only initialize 必须显式绑定 source commit、Switcher/Cavalry launcher/runtime SHA、vendor Team ID 与 expected language，并要求当前用户不同于 source tree owner、Cavalry 精确位于当前用户 `$HOME/Applications/Cavalry.app` 且 bundle/关键 executable 归当前 UID；按固定阶段记录 point/backing-scale、Reduce Motion 和仅 Switcher 自有窗口截图，System Settings 只留无标题几何，permission-blocked 不推断权限，retry-verified 只接受 marker、strict codesign 与当前用户 Application Support `state.json` 的目标回读，禁止读写 TCC、自动改权限或把 drop 当授权事实。
...
2026-08-31: macOS handoff recorder 升级为机器门；非 read-only session 由六个显式 expected 字段绑定源码与产物，拒绝脏/分支 checkout、同 UID source tree、全局 `/Applications` 或非当前用户拥有的 Cavalry，并以 schema 3 manifest / schema 2 checkpoint / schema 3 seal 固化身份。`permission-blocked` 仍只封存真实 Switcher UI；`retry-verified` 必须在记录时回读用户域 marker、strict bundle seal 与当前用户 Application Support 的 `state.json`（target appPath、expected currentLang、非空 operationId），全程不读取/写入 TCC 或驱动 System Settings。
