...
check_tauri_bridge_runtime.js: Tauri bridge fake-DOM 运行合同，执行真实组件和 app 源；覆盖手动安装入口、保留但禁用当前语言的 Select Trigger/popup 占位与显式选择、Managed Legacy 受管英文恢复、旧/新/未知版本只读门禁、只读权限未知的 idle 投影、主任务/Updater/About/窗口控件，锁定更新确认不内嵌 manifest changelog，并证明 About/外链失败进入局部 Toast 而不覆盖 Activity。
