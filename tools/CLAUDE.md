...
check_selection_value_contract.test.js: CI-safe 选择值保护接线合同，冻结双平台共享输入策略的 Combo/编辑器/弹出列表覆盖，与原生 fixture 分层验收。
...
classify_ci_changes.js: GitHub Actions 变更风险分类器；以 PR merge-base、push before/head 或事件强制策略投影 documentation/source/vulnerability/windows/macOS-injector 门，未知源码路径和不可解析 diff 均 fail-closed，tag/workflow_dispatch 永远跑完整闭环。
classify_ci_changes.test.js: CI 风险分类器离线合同；覆盖纯文档、合同/SOP、Windows、macOS、共享翻译表及翻译/输入策略的逐文件双平台门、依赖/workflow、定时漏洞复审、空 diff 与未知源码/自动化路径边界。
check_tauri_build_sop.js: Tauri 打包 SOP、四语 README 用户路径、CI 风险调度与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，验证 workflow 按 classifier 输出选择昂贵证据并由稳定 ci_gate 汇总选中/跳过结果、README 到最新 Release/唯一构建 SOP 的导航，以及 SOP/配置同构的 400×484 固定最小窗口、主窗口无滚动而 Select/任务事件视窗可内部滚动、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability，并验证 `NSAppBundlesUsageDescription` 四语源文件、`bundle.macOS.files` 目标、DMG `产品 + SemVer + 架构` 卷标和可选最终 `.app` readback。
