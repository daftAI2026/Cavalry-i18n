...
release_publish.js: 幂等 GitHub Release 发布器；私有 draft 先补全并逐项回读三项人工安装资产、六项 updater manifest/archive/signature 与四项可复验 sidecar，最后一步才公开；本地生成 SHA256SUMS/ReleaseAssetProvenance，并复核 manifest 语义、SBOM、toolchain 与所有分发字节，冲突 fail-closed。
create_sbom.js / verify_release_provenance.js: 从 npm/Cargo lockfiles 生成确定性 CycloneDX 1.5 SBOM，并复核公开 ReleaseAssetProvenance v5 对 release commit、真实平台签名声明、SBOM、toolchain、九项人工安装/updater 分发字节及 manifest 三平台语义的精确绑定。
