...
check_quick_add_search.cpp: Qt 6.6.3 vendor-free FastQuickAdd model/view 合同；以可控 owner/model fixture 证明精确 FastQuickAddModel 下三语 role 257 alias 投影与内侧 Unicode code-point subsequence filter 不污染 role 0/256 与其他角色，并保留 vendor role 0/257 fuzzy subsequence 命中（含 Behaviour Mixer），覆盖视频词条 Text/Shape/Box/Circle、任意新条目、多词别名、大小写/标点/组合字符/clear、分类多实例、动态插入删除、源替换、幂等挂接、原始搜索框接线、拒绝任意 Qt 列表及 QObject 生命周期，不冒充真实 Cavalry UI 证据；搜索上下文 guard 来自窄 `injector/cavalry_i18n_quick_add_context.h`。
check_quick_add_search.sh: 本地 Quick Add 搜索合同 runner；从 `CAVALRY_QT_PREFIX` 或仓库 `qt_sdk/6.6.3/macos` 选择 SDK，先以 `qmake -query QT_VERSION` 验证 6.6.3，再在临时目录运行 moc/clang++ fixture，不读取或修改真实 Cavalry.app；编译闭包包含共享 Quick Add context。
check_quick_add_display.cpp: vendor-free Quick Add 显示副本合同；以 exact `FastQuickAddModel`/`FastQuickAddDelegate` 复现已读证 Qt 6.6.3 `FastQuickAddItem` 128/8 布局，验证 vendor/version/type/size/alignment/title fail-open、仅 copy title 投影、未知 model/role/index 透传、paint/sizeHint shadow、编辑/help/commit 原 index、model replacement 首绘、英文回退、幂等 wrapper 与 delegate 生命周期。
...
check_classic_quick_add_search.cpp: Qt 6.6.3 vendor-free Classic QuickAddWindow/ListWidget 合同；证明 U+FFFE role0 source 与 query-dependent 标题/英文说明 token 投影、exact search-box guard、独立 itemWidget 标题与说明显示层、多语言 Text/Shape/Box 与 Circle 负例、native filter/no-results、ASCII cleanup 与 `ab`/`ba`/`aa` 顺序匹配、16 项多语言同 locale 原文/投影 `QListWidgetItem` 排序矩阵、延后 itemWidget、动态插入删除排序/reset、幂等及标准 Qt DisplayRole MIME 结果，不冒充 vendor command/custom MIME 真人证据；macOS 已由本地 SDK 验证，Windows 必须由同一 fixture 的 CTest 执行且失败即阻断，待 native CI/live 取得 vendor 证据；搜索框 guard 来自窄 shared context。
check_classic_quick_add_search.sh: 本地 Classic Quick Add 搜索合同 runner；从 `CAVALRY_QT_PREFIX` 或仓库 `qt_sdk/6.6.3/macos` 选择 SDK，先以 `qmake -query QT_VERSION` 验证 6.6.3，再在隔离临时目录运行 moc/clang++ fixture 及 U+FFFE 同 locale 排序矩阵，不读取或修改真实 Cavalry.app；编译闭包包含共享 Quick Add context。
