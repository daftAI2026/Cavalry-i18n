...
check_renderer_contract.js: Renderer 静态合同，冻结 local-only 资源、DOM/API 锚点、macOS Overlay 原生交通灯边界、Item/Native Select/Button Group/Alert 布局、更新 tooltip、四语 Maintenance 文案与两种 English 恢复语义。
check_tauri_bridge_runtime.js: Tauri bridge 运行时测试，在 fake DOM 中按 bridge/ui-text/app 执行真实 renderer 源，覆盖四语标题/状态/Badge、fail-closed 控件、updater tooltip/dialog、Windows residue/UAC、macOS 权限、两种 English 恢复和 durability retry。
check_tauri_build_sop.js: Tauri 打包 SOP 与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，冻结 460×440 默认窗口与 macOS decorations/Overlay/hiddenTitle 原生窗口合同。
