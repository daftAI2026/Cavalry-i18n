...
check_macos_quick_add_inputs.sh: 本地 macOS Quick Add 搜索输入合同 runner；只读链接 Qt 6.6.3/vendor Frameworks，绿色运行生产搜索 guard，红色仅在临时生产源码副本移除 guard 后预期失败。
...
classify_ci_changes.js: GitHub Actions 变更风险分类器；以 PR merge-base、push before/head 或事件强制策略投影 documentation/source/vulnerability/windows/macOS-injector 门，未知源码路径和不可解析 diff 均 fail-closed，tag/workflow_dispatch 永远跑完整闭环；共享 Quick Add 显示适配器、描述输入/策略与生成表进入双平台 native 门。
classify_ci_changes.test.js: CI 风险分类器离线合同；覆盖纯文档、合同/SOP、Windows、macOS、共享翻译表及翻译/输入/Quick Add 显示/搜索/Classic 策略的逐文件双平台门、macOS-only 原生源码、依赖/workflow、定时漏洞复审、空 diff 与未知源码/自动化路径边界。
check_tauri_build_sop.js: Tauri 打包 SOP、四语 README 用户路径、CI 风险调度与配置 contract 测试；除跨平台发布、updater 资产、tag preflight、macOS/Windows 打包门外，验证 workflow 按 classifier 输出选择昂贵证据并由稳定 ci_gate 汇总选中/跳过结果、macOS 原生门实际调用无 vendor Quick Add 搜索/Classic/显示 fixture、Windows CMake/CTest 接入共享搜索 fixture，Quick Add 显示头文件变更同时触发双平台门并使旧 installer provenance 失效，Quick Add 描述生成器/English+三语 JSON/共享 header/生成表进入 package/native/provenance 闭包，四语徽章直接读取最新正式 Release 且禁止本地 JSON/发布后 PR 同步、README 到最新 Release/唯一构建 SOP 的导航，以及 SOP/配置同构的 400×484 固定最小窗口、主窗口无滚动而 Select/任务事件视窗可内部滚动、macOS decorations/Overlay/hiddenTitle 原生窗口合同与显式标题拖动 capability，并验证 `NSAppBundlesUsageDescription` 四语源文件、`bundle.macOS.files` 目标、DMG `产品 + SemVer + 架构` 卷标和可选最终 `.app` readback。
...
build_translator_injector.sh: 同步生成独立 Quick Add 说明索引， 重生成共享翻译表后，以 `-O2/-fno-omit-frame-pointer` 合编 macOS injector 与 TransformTool ABI 适配器；真实运行绑定 `@rpath/libskia.dylib`，无 vendor app 的 CI 只使用不入包的临时链接桩。
