 ...
 * [POS]: tools 的 CI 调度分类器，共享翻译/输入/Quick Add context/display/搜索/Classic 搜索与评分/Quick Add 描述策略必须经过双平台门，macOS-only 原生源码只进入 macOS 门；tag、手动运行、未知路径和不可解析 diff 均 fail-closed
...
const WINDOWS_PATHS = [
  /^injector\/windows\//,
  /^injector\/(?:generated_translations\.inc|generated_quick_add_descriptions\.inc|cavalry_i18n_(?:translation_policy|input_policy|quick_add_context|quick_add_display|search_policy|classic_search|classic_rank|search_descriptions)\.h)$/,
  /^renderer\//,
  /^languages\//,
  /^src-tauri\//,
  /^package(?:-lock)?\.json$/,
