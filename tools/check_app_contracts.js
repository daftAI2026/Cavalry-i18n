...
const { ALLOWED_PREFIXES, cleanupTempDirs, makeTempDir } = require('./test_temp_dir.js');
...
test('contract fixtures stay in direct TEMP children with a finite prefix allowlist', () => {
  const fixture = makeTempDir();
  assert.equal(path.dirname(fixture), fs.realpathSync.native(os.tmpdir()));
  assert.match(path.basename(fixture), /^cavalry-i18n-test-[A-Za-z0-9]+$/);
  assert.deepEqual([...ALLOWED_PREFIXES].sort(), [
  ...
  assert.throws(() => makeTempDir('cavalry-i18n-test-../'), /Unsupported test temporary directory prefix/);
});

test('English refresh and extract stay read-only while apply owns macOS recovery', () => {
  const snapshot = fs.readFileSync(path.join(repoRoot, 'src-tauri', 'src', 'commands', 'snapshot.rs'), 'utf8');
  const apply = fs.readFileSync(path.join(repoRoot, 'src-tauri', 'src', 'commands', 'apply.rs'), 'utf8');
  const snapshotProduction = snapshot.split('#[cfg(test)]', 1)[0];
  assert.doesNotMatch(
    snapshotProduction,
...
test('embedded injector translates dynamic QLabel and QLineEdit text before repaint', () => {
  ...
  const labelDisplayBody = injectorSource.slice(
    injectorSource.indexOf('void translateLabelDisplayText('),
    injectorSource.indexOf('bool paintTextFingerprintMatches(')
  );
  assert.match(
    labelDisplayBody,
    /translatedWidgetText\(lang, label->text\(\)\)[\s\S]*label->setText\(translated\)/,
...
test('release supply-chain gates pin vulnerability inputs, Qt installer identity, and record runner identity', () => {
  ...
  assert.match(workflow, /for attempt in 1 2 3; do[\s\S]*--fetch-timeout=60000[\s\S]*npm audit found a vulnerability; failing closed\./);
  assert.match(workflow, /continuing without a fresh npm advisory report[\s\S]*if \[ "\$audit_available" -eq 1 \]/);
...
test('npm version command prefers the npm CLI path and limits shell fallback to Windows', () => {
  ...
  assert.deepEqual(
    resolveNpmVersionCommand({ env: {}, platform: 'win32', nodePath: 'node.exe' }),
    { command: 'npm', args: ['--version'], shell: true }
  );
  assert.deepEqual(
    resolveNpmVersionCommand({ env: {}, platform: 'darwin', nodePath: '/usr/bin/node' }),
