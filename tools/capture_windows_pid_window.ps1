...
[POS]: tools 的 Windows GUI 取证边界；Edit Shape 与人工 CogPitch 通过有界 exact-HWND 前台门，拒绝预置 bit 26 并保存前后诊断，不创建场景、不依赖 Qt UIA、不运行脚本，禁止坐标/鼠标回退、强杀、固定 sleep 或覆盖证据
...
if ($CaptureScenario -ceq 'CogPitch') {
    ...
    Assert-Condition `
        -Condition (([int]$cogPitchBaseline.translatedSourceMask -band 0x04000000) -eq 0) `
        -Message 'CogPitch baseline contains a pre-set Pitch bit 26; restart the owned clone before collecting evidence.'
}
