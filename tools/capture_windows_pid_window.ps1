...
[POS]: tools 的 Windows GUI 取证边界；Edit Shape 与人工 CogPitch 通过有界 exact-HWND 前台门，Edit Shape 要求六条动作和三条长操作前缀完整命中，CogPitch 拒绝预置 bit 29 并保存前后诊断，不创建场景、不依赖 Qt UIA、不运行脚本，禁止坐标/鼠标回退、强杀、固定 sleep 或覆盖证据
...
if ($CaptureScenario -ceq 'CogPitch') {
    ...
    Assert-Condition `
        -Condition (([int]$cogPitchBaseline.translatedSourceMask -band 0x20000000) -eq 0) `
        -Message 'CogPitch baseline contains a pre-set Pitch bit 29; restart the owned clone before collecting evidence.'
}
...
$requiredTextPathMask = switch ($CaptureScenario) {
    ...
    'TransformHelper' { 0x03C07C00 }
    'EditShapeHelper' { 0x1C0003F0 }
    'CogPitch' { 0x20000000 }
    default { 0 }
}
$diagnosticArguments = @{
