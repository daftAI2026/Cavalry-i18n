...
session_contract.js: R5 manifest/checkpoint/retry 的纯数据合同与验证器；锁定非 read-only 的六项 artifact contract、非英文目标语言、真正空安装证明、阶段序列、身份回读与 seal 结构。
check_contract.test.js: 记录器静态安全合同；锁定阶段枚举、仓库外 session、只截 Switcher PID、空安装门、无 TCC/AX/输入合成及 GEB 契约。

依赖边界
- 复用 `macos-acceptance/path_safety.js` 与 `artifact_identity.js` 的路径和身份原语，不另造第二套证据哈希规则。
- `session_contract.js` 只依赖 Node `path` 与输入数据；现场文件检查、codesign、WindowServer 探针只能由 `record_checkpoint.js` 执行。
